...
import com.sdrerc.v3.infrastructure.dao.UsuarioAuthDAO;
import com.sdrerc.v3.infrastructure.dao.UsuarioEmailOtpDAO;
import com.sdrerc.v3.infrastructure.dao.UsuarioTotpBackupCodeDAO;
import com.sdrerc.v3.security.EmailOtpMailer;
import com.sdrerc.v3.security.PasswordEncoder;
import com.sdrerc.v3.security.TotpSecretCipher;
import com.sdrerc.v3.security.TotpService;
...
/**
 * Port casi literal de com.sdrerc.application.sdrercapp.AutenticacionService (V2). Misma
 * orquestacion, mismos mensajes, mismo umbral de bloqueo (5 intentos / 15 min compartido entre
 * password/TOTP/backup code/OTP-correo). Diferencias respecto a V2:
 * <ul>
 *   <li>{@code @Service} de Spring en vez de instanciacion manual (mismo shape de constructor
 *   con DI explicita que V2 ya tenia).</li>
 *   <li>{@code completarLogin} retorna {@link AuthenticatedUser} (record inmutable, pensado para
 *   claims de JWT) en vez del {@code User} legacy de V2, que tenia campos muertos/vestigiales no
 *   relevantes para V3.</li>
 * </ul>
 */
@Service
public class AutenticacionService {
    ...
    private final UsuarioAuthDAO usuarioDAO;
    private final UsuarioTotpBackupCodeDAO backupCodeDAO;
    private final UsuarioEmailOtpDAO emailOtpDAO;

    public AutenticacionService(UsuarioAuthDAO usuarioDAO, UsuarioTotpBackupCodeDAO backupCodeDAO,
    ...
    public AuthenticatedUser completarLogin(Long idUsuario) throws SQLException {
        if (idUsuario == null) {
            throw new IllegalStateException("Sesión de login inválida.");
        }
        ...
        return new AuthenticatedUser(usuario.getIdUsuario(), usuario.getUsername(),
                usuario.getNombreCompleto(), new ArrayList<>(codigosRol));
    }

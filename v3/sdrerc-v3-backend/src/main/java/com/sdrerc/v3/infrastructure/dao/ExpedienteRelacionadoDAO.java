...
/**
 * Port de com.sdrerc.infrastructure.sdrercapp.dao.ExpedienteRelacionadoDAO (V2), acotado a lo que
 * necesita el panel "Asociar" (compartido por Registro y Asignación, ver
 * {@code com.sdrerc.v3.web.ExpedienteRelacionadoController}): {@link #listarSolicitudesDelGrupo}
 * (candidatos + principal, mismo bloque unificado que ya diseñó V2 para ambos módulos),
 * {@link #asociarRelacionados} (confirmar asociación) y {@link #listarAsociadosConfirmados}
 * (resultado ya confirmado). Quedan fuera de este port: {@code desasociarRelacionado} (quitar una
 * asociación ya confirmada) y {@code obtenerExpedientePrincipalAsociado} (no hace falta: el
 * principal ya viene incluido como primera fila de {@code listarSolicitudesDelGrupo} en V2).
 *
 * <p>Mismas diferencias deliberadas que el resto de DAOs de V3 (ver Javadoc de
 * {@code AsignacionExpedienteDAO}): sin chequeos de compatibilidad retro, excepciones de negocio
 * como {@code IllegalArgumentException}/{@code IllegalStateException} (no {@code SQLException})
 * para que el frontend reciba el mensaje real en vez de un 500 genérico.</p>
 */
@Repository
public class ExpedienteRelacionadoDAO {
     ...
     * Registro/Recepción y Asignación: incluye el expediente PRINCIPAL (el enfocado) más los
     * candidatos pendientes de asociar (misma acta+titular, aún no confirmados) — a propósito NO
     * incluye los ya asociados/confirmados. Ordenado ascendente por fecha de solicitud.
     */
    ...
    public ExpedienteRelacionResultadoDTO asociarRelacionados(
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                Long idMovimientoAsignacion = catalogoLookupDAO.obtenerTipoMovimientoId(conn, MOVIMIENTO_ASIGNACION_ABOGADO);
                for (Long idSeleccionado : idsUnicos) {
                    ...
                    if (!existeExpedienteActivo(conn, idExpedientePrincipal) || !existeExpedienteActivo(conn, idSeleccionado)) {
                        throw new IllegalStateException("Uno de los expedientes seleccionados ya no existe o no está activo.");
                    }
                    ...
                    if (estaRelacionadoComoHijoEnOtroPrincipal(conn, idSeleccionado, idExpedientePrincipal)) {
                        throw new IllegalStateException("El expediente " + idSeleccionado + " ya está asociado a otro expediente principal. Desasócielo antes de volver a relacionarlo.");
                    }
                    OrientacionRelacion orientacion = resolverOrientacionRelacion(conn, idExpedientePrincipal, idSeleccionado);
                    Date fechaVencimientoPrincipal = resolverFechaVencimientoPrincipal(conn, orientacion.idPrincipal, idUsuarioCreador);
                    sincronizarNumeroExpedienteRelacionado(conn, orientacion.idPrincipal, orientacion.idRelacionado, idUsuarioCreador);
                    if (existeRelacionActiva(conn, orientacion.idPrincipal, orientacion.idRelacionado)) {
                        sincronizarFechaVencimientoRelacionado(conn, orientacion.idRelacionado, fechaVencimientoPrincipal, idUsuarioCreador);
                        sincronizarAsignacionDesdePrincipal(conn, orientacion.idPrincipal, orientacion.idRelacionado, idUsuarioCreador, idMovimientoAsignacion, true);
                        expedienteAlertaDAO.marcarAtendidas(conn, orientacion.idRelacionado, Collections.singletonList("Potencial duplicado"), idUsuarioCreador);
                        yaAsociados++;
                        continue;
                    }
                    Long idRelacion = insertarRelacion(conn, orientacion.idPrincipal, orientacion.idRelacionado, idUsuarioCreador, descripcion);
                    sincronizarFechaVencimientoRelacionado(conn, orientacion.idRelacionado, fechaVencimientoPrincipal, idUsuarioCreador);
                    sincronizarAsignacionDesdePrincipal(conn, orientacion.idPrincipal, orientacion.idRelacionado, idUsuarioCreador, idMovimientoAsignacion, true);
                    expedienteAlertaDAO.marcarAtendidas(conn, orientacion.idRelacionado, Collections.singletonList("Potencial duplicado"), idUsuarioCreador);
                    if (idMovimiento != null) {
                        insertarHistorialRelacion(conn, orientacion.idPrincipal, idMovimiento, idUsuarioCreador, idRelacion, descripcion);
                        insertarHistorialRelacion(conn, orientacion.idRelacionado, idMovimiento, idUsuarioCreador, idRelacion, descripcion);
                    }
                    asociados++;
                }
                conn.commit();
        ...
        return new ExpedienteRelacionResultadoDTO(idsRelacionados.size(), asociados, yaAsociados, omitidos, mensaje);
    }
    ...
    private boolean sincronizarAsignacionDesdePrincipal(
        ...
        AsignacionActual asignacionDestino = prepararAsignacionDestino(conn, asignacionPrincipal, sincronizarEstadoOperativo);
        ...
        Long idAsignacion = insertarAsignacionAsociada(conn, idRelacionado, asignacionPrincipal, idUsuarioModificador);
        actualizarResponsablesRelacionado(conn, idRelacionado, asignacionDestino, idUsuarioModificador, actualizarEstado);
        insertarHistorialAsignacionAsociada(conn, idRelacionado, asignacionPrincipal, idMovimiento, idAsignacion, idUsuarioModificador);
        return true;
    }
    ...
    private ExpedienteEstadoActual obtenerEstadoActual(Connection conn, Long idExpediente) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    throw new IllegalStateException("El expediente principal seleccionado ya no está disponible.");
                }
    ...
    private boolean tieneAsignacionCoincidente(Connection conn, Long idExpediente, AsignacionActual asignacion) throws SQLException {
    ...
    private void desactivarAsignacionesActuales(Connection conn, Long idExpediente, Long idUsuarioModificador) throws SQLException {
    ...
    private Long insertarAsignacionAsociada(Connection conn, Long idExpediente, AsignacionActual asignacion, Long idUsuarioModificador) throws SQLException {
    ...
    private OrientacionRelacion resolverOrientacionRelacion(Connection conn, Long idExpedienteA, Long idExpedienteB) throws SQLException {
    ...
    private ExpedienteReferencia obtenerReferenciaExpediente(Connection conn, Long idExpediente) throws SQLException {
        String sql = "SELECT numero_expediente, fecha_registro FROM expediente WHERE id_expediente = ? AND activo = 1";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, idExpediente);
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    throw new IllegalStateException("El expediente seleccionado ya no está disponible.");
                }
    ...
    private Long insertarRelacion(Connection conn, Long idPrincipal, Long idRelacionado, Long idUsuarioCreador, String descripcion) throws SQLException {
    ...
    private void insertarHistorialRelacion(Connection conn, Long idExpediente, Long idMovimiento, Long idUsuarioCreador, Long idRelacion, String descripcion) throws SQLException {
        String sql = "INSERT INTO expediente_historial ("
                + "id_expediente, id_tipo_movimiento, fecha_movimiento, id_usuario_origen, "
    ...
    private Date resolverFechaVencimientoPrincipal(Connection conn, Long idExpedientePrincipal, Long idUsuarioCreador) throws SQLException {
        ...
        Date calculada = Date.valueOf(
                calendarioLaboralService.calcularFechaVencimientoSolicitud(conn, fechaSolicitud.toLocalDate(), null));
        sincronizarFechaVencimientoRelacionado(conn, idExpedientePrincipal, calculada, idUsuarioCreador);
        return calculada;
    }

    private void sincronizarFechaVencimientoRelacionado(Connection conn, Long idExpediente, Date fechaVencimiento, Long idUsuarioModificador) throws SQLException {
    ...
    private void sincronizarNumeroExpedienteRelacionado(Connection conn, Long idPrincipal, Long idRelacionado, Long idUsuarioModificador) throws SQLException {
        if (idPrincipal == null || idRelacionado == null) {
            return;
        }
    ...
    /**
     * Si idExpediente ya está asociado como relacionado de otro principal (relación activa),
     * devuelve ese principal en vez de idExpediente; recorre la cadena por si hubiera más de un
     * salto. Evita que un expediente ya asociado se use como principal de una nueva relación.
     */
    ...
    private ExpedienteRelacionadoDTO mapAsociado(Connection conn, ResultSet rs) throws SQLException {
        LocalDate fechaVencimiento = toLocalDate(rs.getDate("fecha_vencimiento"));
        return new ExpedienteRelacionadoDTO(
                getLongOrNull(rs, "id_expediente"),
                rs.getString("numero_expediente"),
                rs.getString("numero_expediente_sgd"),
    ...
    private static final class AsignacionActual {
        ...
        private AsignacionActual(
            ...
            if (idUsuario == null || idEtapa == null) {
                throw new IllegalStateException("La asignación vigente del expediente principal está incompleta.");
            }

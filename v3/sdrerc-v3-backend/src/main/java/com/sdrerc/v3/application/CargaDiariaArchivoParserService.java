...
/**
 * Port casi literal de com.sdrerc.application.sdrercapp.CargaDiariaArchivoParserService (V2):
 * mismo motor de detección de encabezados por alias, mismas reglas de fecha/canal derivadas. Único
 * cambio real: recibe {@link InputStream} + nombre de archivo (viene de un multipart HTTP) en vez
 * de {@link java.io.File} (V2 usaba {@code JFileChooser}), sin cambiar ninguna regla de negocio.
 */
@Service
public class CargaDiariaArchivoParserService {
    ...
    public List<CargaDiariaPreviewDTO> leerArchivo(InputStream contenido, String nombreArchivo) throws IOException {
    ...
    private List<CargaDiariaPreviewDTO> leerExcel(InputStream contenido) throws IOException {
        try (Workbook workbook = WorkbookFactory.create(contenido)) {
    ...
    private List<CargaDiariaPreviewDTO> leerCsv(InputStream contenido) throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(contenido, StandardCharsets.UTF_8))) {

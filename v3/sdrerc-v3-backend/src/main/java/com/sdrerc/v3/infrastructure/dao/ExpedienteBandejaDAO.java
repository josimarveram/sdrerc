...
/**
 * Port literal de com.sdrerc.infrastructure.sdrercapp.dao.ExpedienteBandejaDAO (V2), mismo SQL,
 * misma vista {@code vw_expediente_bandeja} (ya existe en la BD compartida). Único cambio real:
 * la conexión se obtiene de un {@link DataSource} inyectado por Spring.
 */
@Repository
public class ExpedienteBandejaDAO {
    ...
    public ExpedienteBandejaDAO(DataSource dataSource, CalendarioLaboralService calendarioLaboralService) {
    ...
    public List<ExpedienteBandejaDTO> buscar(
        ...
        if (hasText(etapaCodigo)) {
            ...
            params.add(etapaCodigo.trim().toUpperCase(Locale.ROOT));
        }
    ...
    private static String grupoFamiliar(boolean grupoFamiliar, String criterio, String observacion) {
        if (grupoFamiliar) {
            return "Sin Alerta";

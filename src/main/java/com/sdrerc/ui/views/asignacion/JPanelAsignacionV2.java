...
public class JPanelAsignacionV2 extends JPanel {
    ...
    private final JButton btnCancelarAsignacionCarta = new JButton("Cancelar");
    ...
    private AppV2SideActionPanel crearPanelCartasRespuesta() {
        ...
        panel.setFooter(crearAccionesDestinoOperativoCarta());
        return panel;
    }
    ...
    private JPanel crearPanelDocumentosRelacionados() {
        ...
        documentosRelacionadosWrapper.add(documentosRelacionadosScroll, BorderLayout.CENTER);
    ...
    private void configurarTablaDocumentosRelacionados() {
        ...
        documentosRelacionadosTable.setAutoResizeMode(JTable.AUTO_RESIZE_SUBSEQUENT_COLUMNS);
        ...
        documentosRelacionadosTable.getColumnModel().getColumn(5).setPreferredWidth(160);
        documentosRelacionadosTable.getColumnModel().getColumn(5).setMinWidth(140);
        documentosRelacionadosTable.getColumnModel().getColumn(6).setPreferredWidth(140);
        documentosRelacionadosTable.getColumnModel().getColumn(6).setMinWidth(120);
        documentosRelacionadosTable.getColumnModel().getColumn(7).setPreferredWidth(160);
        documentosRelacionadosTable.getColumnModel().getColumn(7).setMinWidth(140);
    ...
    private void asociarRelacionadosRapido() {
        ...
        if (seleccion.esSeleccionMultiple()) {
            ...
            if (resolucion.ambiguo) {
                mostrarInfo("Se detectó al menos un expediente con número entre las solicitudes marcadas como "
    ...
    private void generarNumeroExpedienteSeleccionado() {
        ...
        final Long idExpedienteFoco = item.getIdExpediente();
        SwingWorker<String, Void> worker = new SwingWorker<String, Void>() {
            @Override
            protected String doInBackground() throws Exception {
                return asignacionService.generarNumeroExpediente(idExpedienteFoco);
            }
    ...
    private void generarNumerosParaMarcados(List<Long> ids) {
        ...
        setTrabajando(true, "Generando números de expediente...");
        final Long idPrincipalFoco = idExpedienteDocumentosRelacionados;
        SwingWorker<List<String>, Void> worker = new SwingWorker<List<String>, Void>() {
            @Override
            protected List<String> doInBackground() throws Exception {
                List<String> numeros = new ArrayList<>();
                for (Long id : ids) {
                    numeros.add(asignacionService.generarNumeroExpediente(id));
                }
                return numeros;
            }
    ...
    /**
     * El principal vigente (idPrincipalElegidoRelacionado) resuelve tanto el caso automatico (ningun
     * expediente con numero, usa el enfocado como respaldo) como el caso manual (1 o mas con numero:
     * el usuario elige marcando la casilla del que corresponda, ninguna queda bloqueada).
     */
    ...
    private void cargarAbogadosDestinoOperativoCarta(final Long idExpedienteSolicitado, final Long idEquipo) {
        SwingWorker<List<UsuarioAsignableDTO>, Void> worker = new SwingWorker<List<UsuarioAsignableDTO>, Void>() {
            @Override
            protected List<UsuarioAsignableDTO> doInBackground() throws Exception {
    ...
    private boolean seleccionarAbogadoCartaPorId(Long idAbogado) {
        if (idAbogado == null) {
            return false;
        }
        for (int i = 0; i < cmbAbogadoCarta.getItemCount(); i++) {
            UsuarioItem item = cmbAbogadoCarta.getItemAt(i);
            if (item.usuario != null && idAbogado.equals(item.usuario.getIdUsuario())) {
                cmbAbogadoCarta.setSelectedIndex(i);
                return true;
            }
        }
        return false;
    ...
    private void actualizarSupervisorCarta() {
        ...
        lblSupervisorCarta.setText(abogado == null || abogado.getSupervisorNombre().isEmpty() ? "-" : abogado.getSupervisorNombre());
    }
    ...
    private void registrarAsignacionCarta() {
        ...
        final EquipoAsignacionDTO equipo = equipoAnalisisCartaDTO;
        final AsignacionCartaRespuestaDTO cartaActual = cartaRespuestaSeleccionada;
        btnRegistrarAsignacionCarta.setEnabled(false);
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(
                            ...
                            JOptionPane.INFORMATION_MESSAGE);
                    if (cartaActual != null && cartaActual.equals(cartaRespuestaSeleccionada)) {
                        cargarDetalleCartaRespuestaSeleccionada(cartaActual);
                    }
                } catch (Exception ex) {
                    mostrarError("No se pudo registrar la asignación.", ex);
                } finally {
                    btnRegistrarAsignacionCarta.setEnabled(true);
                }
            }
        };
        worker.execute();
    ...
    /**
     * El expediente PRINCIPAL real para efectos de asociacion no es necesariamente el expediente
     * enfocado en la bandeja (el usuario puede haber hecho doble clic sobre una solicitud SIN
     * numero, ej. una Reconsideracion). Si ningun expediente del grupo tiene numero, el enfocado
     * actua como principal de respaldo. Si al menos uno del grupo YA TIENE numero, ninguna casilla
     * queda bloqueada: el usuario debe marcar activamente cual de los "con numero" es el principal.
     */
    private void recalcularPrincipalRelacionadoReal(
        ...
        if (!idsConNumeroRelacionado.isEmpty()) {
            idPrincipalRelacionadoReal = null;
    ...
    private int contarCandidatosSinNumeroRelacionados() {
        int total = 0;
        for (DocumentoRelacionadoFila fila : documentosRelacionadosPanel) {
            if (fila != null && requiereDecisionNumeroCandidato(fila.getExpediente())) {
                total++;
            }
        }
        return total;
            ...
            protected void done() {
                ...
                try {
                    ...
                    for (ExpedienteRelacionadoDTO relacionado : relacionados) {
                        boolean esCandidatoConNumero = tieneNumeroExpediente(relacionado);
                        DocumentoRelacionadoFila fila = new DocumentoRelacionadoFila(relacionado, esCandidatoConNumero);
                        documentosRelacionadosPanel.add(fila);
                        if (!esCandidatoConNumero) {
                            totalCandidatos++;
                        }
    ...
    private void eliminarAlertaGrupoFamiliarSeleccion() {
        ...
        setTrabajando(true, "Eliminando alerta...");
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() throws Exception {
            ...
            protected void done() {
                try {
                    ...
                    mostrarError("No se pudo eliminar la alerta de Posible Grupo Familiar.", ex);
                } finally {
                    setTrabajando(false, null);
                }
            }
    ...
    private void aplicarEstadoRecepcion(JLabel label, String estado) {
        ...
        } else if ("Pendiente de recibir".equalsIgnoreCase(text) || "Sin abogado asignado".equalsIgnoreCase(text)) {
    ...
    private static final class DocumentoRelacionadoFila {
        ...
        private DocumentoRelacionadoFila(ExpedienteRelacionadoDTO expediente, boolean esCandidatoConNumero) {

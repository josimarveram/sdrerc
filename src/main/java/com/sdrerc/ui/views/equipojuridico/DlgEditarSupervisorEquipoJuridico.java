...
public class DlgEditarSupervisorEquipoJuridico extends JDialog {
    ...
    public DlgEditarSupervisorEquipoJuridico(
            Window parent,
            EquipoJuridicoConsultaService consultaService,
            EquipoJuridicoService equipoJuridicoService,
            Long supervisorId) {
        super(parent, "Editar supervisor", ModalityType.APPLICATION_MODAL);
    ...
    private void construirInterfaz() {
        ...
        agregarCombo(panel, gbc, 6, "Estado", cboEstado);
    ...
    private void cargarDatos() {
        try {
            detalle = consultaService.obtenerDetalleSupervisor(supervisorId);
            cargarTiposPersonal();
            pintarDetalle();
        } catch (Exception ex) {
    ...
    private void guardar() {
        try {
            EquipoJuridicoSupervisorUpdateRequest request = construirRequest();
            consultaService.actualizarSupervisorEquipoJuridico(request);
            guardado = true;
            JOptionPane.showMessageDialog(this, "Supervisor actualizado correctamente.");
            dispose();
        } catch (Exception ex) {
    ...
    private EquipoJuridicoSupervisorUpdateRequest construirRequest() {
        validarCampos();

        EquipoJuridicoSupervisorUpdateRequest request = new EquipoJuridicoSupervisorUpdateRequest();
        request.setSupervisorId(detalle.getAbogadoId());

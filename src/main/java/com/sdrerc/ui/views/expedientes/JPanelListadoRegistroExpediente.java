...
public class JPanelListadoRegistroExpediente extends javax.swing.JPanel {
    ...
    private void exportarListadoRecepcionExcel()
    {
        ...
        fileChooser.setFileFilter(new FileNameExtensionFilter("Archivo Excel (*.xlsx)", "xlsx"));
        fileChooser.setSelectedFile(new File(nombreArchivoExcelRecepcion()));
        ...
        try {
            escribirExcelRecepcion(archivo, filasExportacion);
            JOptionPane.showMessageDialog(this, "Archivo Excel generado correctamente.");
        } catch (IOException ex) {
            Logger.getLogger(JPanelListadoRegistroExpediente.class.getName()).log(Level.WARNING, "No se pudo exportar listado de recepcion", ex);
            JOptionPane.showMessageDialog(this, "No se pudo exportar el archivo Excel.");
        }
    }

    private String nombreArchivoExcelRecepcion()
    {
        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        return "recepcion_solicitudes_" + timestamp + ".xlsx";
    }
    ...
    private void escribirExcelRecepcion(File archivo, List<Integer> filasExportacion) throws IOException
    {
    ...
    private class ExpedienteCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(
            ...
            if (modelColumn == COL_TITULAR && value instanceof TitularListadoValue) {
                ...
                label.setToolTipText((modelColumn == COL_REFERENCIA || modelColumn == COL_ACTA || modelColumn == COL_TITULAR || modelColumn == COL_PROCEDIMIENTO_REGISTRAL || modelColumn == COL_TIPO_DOCUMENTO) ? texto : null);
            }
            label.setBorder(BorderFactory.createEmptyBorder(0, 8, 0, 8));

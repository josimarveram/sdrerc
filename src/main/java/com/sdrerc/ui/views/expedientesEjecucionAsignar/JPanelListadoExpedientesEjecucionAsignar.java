 ...
 * @author betom
 */
public class JPanelListadoExpedientesEjecucionAsignar extends javax.swing.JPanel implements Scrollable {
     ...
     */
    public JPanelListadoExpedientesEjecucionAsignar() {
        initComponents();
        ...
        cargarComboEstados();
        configurarListadoEjecucionAsignarPremium();
        buscarExpedientes();
    }
    ...
    private void cargarMapaCatalogo(int idCatalogo, Map<Integer, String> destino)
    {
      ...
      private void buscarExpedientes() 
      {
        try 
        {
            ...
            List<Expediente> lista = expedienteEjecucionAsignacionService.ListarExpedientesEjecucion(87);
            List<Expediente> filtrada = filtrarPorRangoFechas(lista, obtenerFechaDesde(), obtenerFechaHasta());
            filtrada = filtrarBusquedaLocal(filtrada, campo, valor, idestado);
            cargarTablaNueva(filtrada);
            mostrarMensajeListado(filtrada.isEmpty() ? "No se encontraron expedientes de ejecución con los filtros seleccionados." : " ");
        } 
        catch (Exception e) {
            cargarTablaNueva(java.util.Collections.<Expediente>emptyList());
            mostrarMensajeListado("No se pudo cargar el listado de expedientes de ejecución.");
        }
      }
    ...
    {
        DefaultTableModel model = crearModeloTablaExpedientesEjecucionAsignar();
        model.setRowCount(0);
        ...
        for (Expediente e : principales) {
            model.addRow(crearFilaTablaExpediente(e, TIPO_FILA_PRINCIPAL, 0));
        }
        jTable1.setModel(model);
        configurarTablaExpedientesEjecucionAsignar();
    }

    private DefaultTableModel crearModeloTablaExpedientesEjecucionAsignar()
    {
        String[] columnas = {
    ...
    private Object[] crearFilaTablaExpediente(Expediente e, String tipoFila, int idPadre)
    {
    ...
    private List<Expediente> filtrarBusquedaLocal(List<Expediente> lista, String campo, String valor, int estado)
    {
        String criterio = textoSeguro(valor).toUpperCase(Locale.ROOT);
        List<Expediente> filtrada = new ArrayList<>();
        for (Expediente expediente : lista) {
            if (estado > 0 && expediente.getEstado() != estado) {
                continue;
            }
            if (!criterio.isEmpty() && !obtenerValorBusquedaLocal(expediente, campo).toUpperCase(Locale.ROOT).contains(criterio)) {
                continue;
            }
            filtrada.add(expediente);
        }
    ...
    private String obtenerValorBusquedaLocal(Expediente expediente, String campo)
    {
        switch (textoSeguro(campo)) {
            case "NUMERO_TRAMITE_DOCUMENTO":
                return textoSeguro(expediente.getNumeroTramiteDocumento());
            case "NUMERO_DOCUMENTO":
                return textoSeguro(expediente.getNumeroDocumento());
            case "NUMERO_ACTA":
                return textoSeguro(expediente.getNumeroActa());
            case "TIPO_SOLICITUD":
                return obtenerDescripcionCatalogo(tiposSolicitudPorId, expediente.getTipoSolicitud());
            case "DNI_REMITENTE":
                return textoSeguro(expediente.getDniRemitente());
            case "APELLIDO_NOMBRE_REMITENTE":
                return textoSeguro(expediente.getApellidoNombreRemitente());
            case "TIPO_PROCEDIMIENTO_REGISTRAL":
                return obtenerDescripcionCatalogo(procedimientosPorId, expediente.getTipoProcedimientoRegistral());
            case "ABOGADO_DESIGNADO":
                return textoSeguro(expediente.getAbogadoDesignado());
            default:
                return "";
        }
    }

    private void configurarListadoEjecucionAsignarPremium()
    {
    ...
    private JPanel crearHeader()
    {
        ...
        JLabel title = new JLabel("Bandeja de ejecución");
        title.setFont(new Font("Segoe UI", Font.BOLD, 24));
        title.setForeground(new Color(25, 52, 84));

        JLabel subtitle = new JLabel("Consulte los expedientes recibidos pendientes de asignación para ejecución.");
    ...
    private JButton crearBotonExportarExcel()
    {
        ...
        button.addActionListener(e -> exportarListadoExpedientesEjecucionAsignarExcel());
        return button;
    }

    private void exportarListadoExpedientesEjecucionAsignarExcel()
    {
        ...
        fileChooser.setSelectedFile(new File(nombreArchivoExcelExpedientesEjecucionAsignar()));
        ...
        try {
            escribirExcelExpedientesEjecucionAsignar(archivo, filasExportacion);
            JOptionPane.showMessageDialog(this, "Archivo Excel generado correctamente.");
        } catch (IOException ex) {
            Logger.getLogger(JPanelListadoExpedientesEjecucionAsignar.class.getName()).log(Level.WARNING, "No se pudo exportar listado de Bandeja de ejecución", ex);
            JOptionPane.showMessageDialog(this, "No se pudo exportar el archivo Excel.");
        }
    }

    private String nombreArchivoExcelExpedientesEjecucionAsignar()
    {
        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        return "expedientes_ejecucion_asignar_" + timestamp + ".xlsx";
    }
    ...
    private void escribirExcelExpedientesEjecucionAsignar(File archivo, List<Integer> filasExportacion) throws IOException
    {
    ...
    private JPanel crearCardResultados()
    {
        ...
        JPanel header = new JPanel(new BorderLayout());
        header.setOpaque(false);
        JLabel title = new JLabel("Listado de Bandeja de ejecución");
        ...
        superior.add(crearPanelFiltrosPorColumnaExpedientesEjecucionAsignar(), BorderLayout.CENTER);
    ...
    private void alternarDocumentosAsociados(int viewRow)
    {
        ...
        if (expedientesExpandidos.contains(idExpediente)) {
            ...
            for (Expediente asociado : asociados) {
                model.insertRow(insertIndex++, crearFilaTablaExpediente(asociado, TIPO_FILA_DETALLE, idExpediente));
            }
        }
        if (paginationHelper != null) {
    ...
    private JPanel crearPanelFiltrosPorColumnaExpedientesEjecucionAsignar()
    {
        configurarFiltrosPorColumnaExpedientesEjecucionAsignar();
    ...
    private void configurarFiltrosPorColumnaExpedientesEjecucionAsignar()
    {
    ...
    private void configurarTablaExpedientesEjecucionAsignar()
    {
        ...
        jTable1.setDefaultRenderer(Object.class, new ExpedienteEjecucionAsignarCellRenderer());
        configurarRendererPlazoAtencion();
        configurarOrdenamientoTablaExpedientesEjecucionAsignar();
    ...
    private void configurarOrdenamientoTablaExpedientesEjecucionAsignar()
    {
    ...
    private int obtenerIdTecnicoFiltroEjecucionAsignar()
    {
    ...
    private class ExpedienteEjecucionAsignarCellRenderer extends DefaultTableCellRenderer {
        @Override
        public Component getTableCellRendererComponent(
       ...
       if (evt.getClickCount() == 1 && columna >= 0 && jTable1.convertColumnIndexToModel(columna) == COL_EXPANDIR)
       {
        ...
        if (evt.getClickCount() == 2 && jTable1.getSelectedRow() != -1)
        {
            ...
            if (fila >= 0)
            {
                ...
                JPanelRegistrarExpedientesEjecucionAsignar panel = new JPanelRegistrarExpedientesEjecucionAsignar();
                try
                {
                ...
                {
                    Logger.getLogger(JPanelListadoExpedientesEjecucionAsignar.class.getName()).log(Level.SEVERE, null, ex);
                }

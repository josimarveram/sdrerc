...
public class JPanelBandejaExpedientesNueva extends JPanel {
    ...
    public void refrescar(Long idExpedienteAReseleccionar) {
        buscar(false, idExpedienteAReseleccionar);
    }
    ...
    private void buscar(boolean conservarFiltroAlerta) {
        buscar(conservarFiltroAlerta, null);
    }

    /**
     * Igual que buscar(...), pero si idExpedienteAReseleccionar no es null, reselecciona esa fila
     * despues de recargar la tabla y reabre el panel lateral en la misma lengueta si estaba
     * abierto antes. cargarTabla(...) cierra el panel (ocultarPanelRecepcion()) y limpia la
     * seleccion de forma incondicional; sin este parametro, acciones que refrescan la bandeja
     * (ej. asociar grupo familiar) lo cerrarian de golpe.
     */
    private void buscar(boolean conservarFiltroAlerta, final Long idExpedienteAReseleccionar) {
            ...
            @Override
                protected void done() {
                    try {
                        ...
                        cargarTabla(expedientes, idExpedienteAReseleccionar);
                    } catch (Exception ex) {
                        mostrarError(ex);
                    } finally {
                        if (secuencia == secuenciaBusqueda.get()) {
    ...
    private void cargarTabla(List<ExpedienteBandejaDTO> expedientes) {
        cargarTabla(expedientes, null);
    }

    private void cargarTabla(List<ExpedienteBandejaDTO> expedientes, Long idExpedienteAReseleccionar) {
        boolean panelAbiertoAntes = idExpedienteAReseleccionar != null
                && splitBandeja != null && splitBandeja.isSideVisible();
        boolean expandidoAntes = panelAbiertoAntes && splitBandeja.isSideExpanded();
        String panelActivoAntes = panelRecepcionActivo;
        ocultarPanelRecepcion();
        principalesExpandidos.clear();
        principalesCargando.clear();
        asociadosCacheRegistro.clear();
        ...
        int modelRow = indiceFilaPrincipalRegistro(idExpedienteAReseleccionar);
        if (modelRow >= 0) {
        ...
        if (panelAbiertoAntes && panelActivoAntes != null) {
            mostrarPanelRecepcion(panelActivoAntes, expandidoAntes);
        }
    }
    ...
    private final class JPanelRegistrarGrupoFamiliarV2 extends AppV2SideActionPanel {
        ...
        private final JButton btnRegistrarSeleccionados = new JButton("Asociar seleccionados");
        private final JButton btnLimpiar = new JButton("Limpiar selección");

        private final DefaultTableModel modeloIntegrantes = new DefaultTableModel(
        ...
        private final JTable tablaIntegrantes = new AppV2Table(modeloIntegrantes);
        private final AppV2TablePanel panelTablaIntegrantes = new AppV2TablePanel(
                tablaIntegrantes, "Sin coincidencias detectadas",
        ...
        private JPanelRegistrarGrupoFamiliarV2() {
            ...
            JLabel ayudaDeteccion = new JLabel("<html>Marque las personas que realmente pertenecen a la misma familia. "
                    + "Solo se asigna \"Sí\" a Grupo Familiar; no se asocian expedientes entre sí ni se heredan "
            ...
            contentDeteccion.add(panelTablaIntegrantes, BorderLayout.CENTER);
            seccionDeteccion.addContent(contentDeteccion);
            addSection(seccionDeteccion);
            ...
            AppV2SearchAutocompleteSupport.attach(
                    ...
                    candidato -> valorUiGrupoFamiliar(candidato.getTitular()) + " — "
                            + valorUiGrupoFamiliar(candidato.getNumeroExpediente()),
            ...
            tablaIntegrantes.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
            AppV2TableColumnSizer.applyWidths(tablaIntegrantes, 34, 130, 150, 130, 150);
            modeloIntegrantes.addTableModelListener(evento -> {
                if (evento.getColumn() == 0) {
                    actualizarBotonAsociarGrupoFamiliar();
                }
            });
            AppV2ColumnFilterSupport.install(
                    "RegistroGrupoFamiliarCandidatos",
                    tablaIntegrantes,
                    panelTablaIntegrantes.getScrollPane(),
                    panelTablaIntegrantes,
                    null,
                    0);
            ...
            btnRegistrarSeleccionados.setFont(AppV2Theme.fontBold(AppV2Theme.FONT_SIZE_BASE));
            btnLimpiar.setFont(AppV2Theme.fontPlain(AppV2Theme.FONT_SIZE_BASE));
            btnRegistrarSeleccionados.addActionListener(e -> asociarMarcadosMasivo());
            btnLimpiar.addActionListener(e -> limpiarSeleccion());
            btnAsociarGrupoFamiliar.addActionListener(e -> asociarGrupoFamiliarDeteccion());
            acciones.add(btnRegistrarSeleccionados);
            acciones.add(btnLimpiar);
            setFooter(acciones);
            actualizarEstado();
        }
        ...
        private void actualizarEstado() {
            ...
            lblEstado.setText(seleccionados >= 2
                    ? "Listo para asociar los " + seleccionados + " marcados entre sí."
                    : "Marque 2 o más expedientes en la bandeja para asociarlos directamente.");
            btnRegistrarSeleccionados.setEnabled(seleccionados >= 2);
            btnLimpiar.setEnabled(seleccionados > 0);
        }

        private void asociarMarcadosMasivo() {
            List<Long> marcados = new ArrayList<Long>(obtenerIdsGrupoFamiliarSeleccionados());
            if (marcados.size() < 2) {
                JOptionPane.showMessageDialog(
                        JPanelBandejaExpedientesNueva.this,
                        "Marque al menos 2 expedientes en la bandeja para asociarlos como un mismo grupo familiar.",
                        "Grupo Familiar",
                        JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            Long idPrincipal = marcados.get(0);
            List<Long> resto = marcados.subList(1, marcados.size());
            int confirmacion = JOptionPane.showConfirmDialog(
                    JPanelBandejaExpedientesNueva.this,
                    "Se asociarán " + marcados.size() + " expedientes marcados como un mismo grupo familiar.\n"
            ...
            SwingWorker<GrupoFamiliarResultadoDTO, Void> worker = new SwingWorker<GrupoFamiliarResultadoDTO, Void>() {
                ...
                protected GrupoFamiliarResultadoDTO doInBackground() throws Exception {
                    return grupoFamiliarService.asociarGrupoFamiliar(idPrincipal, resto);
                }
                ...
                protected void done() {
                    try {
                        ...
                        JOptionPane.showMessageDialog(
                                JPanelBandejaExpedientesNueva.this,
                                ex.getMessage() == null ? "No se pudo asociar el grupo familiar." : ex.getMessage(),
                                "Grupo Familiar",
        ...
        private void cargarPosiblesIntegrantes(ExpedienteBandejaDTO expedientePrincipal) {
            ...
            if (idExpediente == null) {
                candidatosActuales = new ArrayList<GrupoFamiliarCandidatoDTO>();
                modeloIntegrantes.setRowCount(0);
                panelTablaIntegrantes.setEmpty(true);
                lblEstadoDeteccion.setText("Seleccione un expediente en la bandeja.");
            ...
            SwingWorker<List<GrupoFamiliarCandidatoDTO>, Void> worker =
                ...
                protected void done() {
                    ...
                    try {
                        candidatosActuales = get();
                    } catch (Exception ex) {
                        candidatosActuales = new ArrayList<GrupoFamiliarCandidatoDTO>();
                        lblEstadoDeteccion.setText(mensajeError(ex));
                    }
                    modeloIntegrantes.setRowCount(0);
                    for (GrupoFamiliarCandidatoDTO candidato : candidatosActuales) {
                        modeloIntegrantes.addRow(new Object[]{
                                Boolean.TRUE,
                                valorUiGrupoFamiliar(candidato.getNumeroExpediente()),
                                valorUiGrupoFamiliar(candidato.getTitular()),
                                DisplayNameMapperV2.etapa(candidato.getEtapaCodigo()) + " / "
                                        + DisplayNameMapperV2.estado(candidato.getEstadoCodigo()),
                                valorUiGrupoFamiliar(candidato.getAbogadoAsignado())
                        });
                    }
                    if (candidatosActuales.isEmpty()) {
                        lblEstadoDeteccion.setText("No se detectaron posibles integrantes por apellidos.");
                    } else {
                        lblEstadoDeteccion.setText(candidatosActuales.size() + " posible(s) integrante(s) detectado(s).");
                    }
                    AppV2TableColumnSizer.sizeToContent(tablaIntegrantes);
                    panelTablaIntegrantes.setEmpty(candidatosActuales.isEmpty());
                    actualizarBotonAsociarGrupoFamiliar();
                }
            };
            worker.execute();
        ...
        private void agregarPosibleIntegranteManual(GrupoFamiliarCandidatoDTO candidato) {
            ...
            for (GrupoFamiliarCandidatoDTO existente : candidatosActuales) {
                if (candidato.getIdExpediente().equals(existente.getIdExpediente())) {
                    JOptionPane.showMessageDialog(
                            JPanelBandejaExpedientesNueva.this,
                            "Esa solicitud ya está en la lista de posibles integrantes.",
                            "Grupo Familiar",
                            JOptionPane.INFORMATION_MESSAGE);
                    return;
                }
        ...
        private void cargarGrupoActual(Long idExpediente) {
            lblEstadoGrupoActual.setText("Consultando grupo familiar...");
        ...
        private void actualizarBotonAsociarGrupoFamiliar() {
            int marcados = 0;
            for (int i = 0; i < modeloIntegrantes.getRowCount(); i++) {
                if (Boolean.TRUE.equals(modeloIntegrantes.getValueAt(i, 0))) {
                    marcados++;
                }
            }
            if (marcados <= 0) {
                ...
                btnAsociarGrupoFamiliar.setToolTipText(
                        "Marque al menos una coincidencia para habilitar la asociación.");
                return;
            }
        ...
        private void asociarGrupoFamiliarDeteccion() {
            if (expedienteFocoGrupoFamiliar == null || expedienteFocoGrupoFamiliar.getIdExpediente() == null) {
                return;
            }
            List<Long> ids = new ArrayList<Long>();
            for (int i = 0; i < modeloIntegrantes.getRowCount() && i < candidatosActuales.size(); i++) {
                if (Boolean.TRUE.equals(modeloIntegrantes.getValueAt(i, 0))) {
                    Long idCandidato = candidatosActuales.get(i).getIdExpediente();
                    if (idCandidato != null) {
                        ids.add(idCandidato);
                    }
                }
            ...
            if (ids.isEmpty()) {
                JOptionPane.showMessageDialog(
                        JPanelBandejaExpedientesNueva.this,
                        "Marque al menos un posible integrante para asociar.",
                        "Grupo Familiar",
                        JOptionPane.INFORMATION_MESSAGE);
                return;
            }
        ...
        public void setEnabled(boolean enabled) {
            ...
            btnRegistrarSeleccionados.setEnabled(enabled && contarIdsGrupoFamiliarSeleccionados() >= 2);
    ...
    private static String valorUiGrupoFamiliar(String value) {
    ...
    private final class JPanelAsociarDuplicadosRecepcionV2 extends AppV2SideActionPanel {
        ...
        private final JButton btnAsociarRapido = new JButton("Asociar al principal");
        ...
        private JPanelAsociarDuplicadosRecepcionV2() {
            ...
            JLabel ayudaTabla = new JLabel("<html>Se muestran todas las solicitudes con la misma acta y titular, incluida "
            ...
            duplicadosWrapper.add(duplicadosScroll, BorderLayout.CENTER);
            ...
            btnAsociarRapido.setToolTipText(
                    "Asocia únicamente las solicitudes marcadas en \"Solicitudes asociadas\" al expediente principal.");
        ...
        private void cargarDuplicados(ExpedienteBandejaDTO expedientePrincipal) {
            ...
            SwingWorker<List<ExpedienteRelacionadoDTO>, Void> worker = new SwingWorker<List<ExpedienteRelacionadoDTO>, Void>() {
                ...
                protected void done() {
                    ...
                    int totalCandidatos = contarCandidatosPendientes();
        ...
        private boolean esPrincipal(ExpedienteRelacionadoDTO item) {
            return item != null
                    && idExpedientePrincipalDuplicados != null
                    && idExpedientePrincipalDuplicados.equals(item.getIdExpediente());
        }
        ...
        private boolean requiereDecisionNumeroCandidato(ExpedienteRelacionadoDTO item) {
            if (item == null) {
                return false;
            }
            return "REGISTRO".equalsIgnoreCase(item.getEtapaCodigo())
        ...
        private void actualizarEstadoBoton() {
            ...
            if (marcados <= 0) {
                btnAsociarRapido.setEnabled(false);
                btnAsociarRapido.setText("Sin relacionados marcados");
                return;
            }
            btnAsociarRapido.setEnabled(true);
            btnAsociarRapido.setText("Asociar al principal (" + marcados + ")");
        }
        ...
        private void actualizarDecisionNumero() {
            ...
            for (Long id : obtenerIdsMarcados()) {
                for (ExpedienteRelacionadoDTO item : duplicadosActuales) {
                    if (id.equals(item.getIdExpediente()) && requiereDecisionNumeroCandidato(item)) {
                        marcadosSinNumero++;
                    }
                }
            }
        ...
        private void generarNumeroExpediente() {
            List<Long> idsMarcadosSinNumero = new ArrayList<Long>();
            for (Long id : obtenerIdsMarcados()) {
                for (ExpedienteRelacionadoDTO item : duplicadosActuales) {
                    if (id.equals(item.getIdExpediente()) && requiereDecisionNumeroCandidato(item)) {
                        idsMarcadosSinNumero.add(id);
                    }
                }
            }
            if (!idsMarcadosSinNumero.isEmpty()) {
                generarNumerosParaMarcados(idsMarcadosSinNumero);
                return;
            }
        ...
        private void generarNumerosParaMarcados(List<Long> ids) {
            int confirmacion = JOptionPane.showConfirmDialog(
                    JPanelBandejaExpedientesNueva.this,
                    "Se generará un número de expediente NUEVO e independiente para cada una de las " + ids.size()
                            + " solicitud(es) marcada(s); no se asociarán al expediente principal.\n"
                            + "¿Desea continuar?",
                    "Generar número de expediente",
                    JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE);
            ...
            SwingWorker<List<String>, Void> worker = new SwingWorker<List<String>, Void>() {
                ...
                protected List<String> doInBackground() throws Exception {
                    ...
                    for (Long id : ids) {
                        numeros.add(asignacionExpedienteServiceRecepcion.generarNumeroExpediente(id));
                    }
                    return numeros;
                }

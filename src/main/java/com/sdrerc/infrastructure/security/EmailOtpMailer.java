 ...
 *
 * <p>Solucion interina mientras se define un SMTP institucional dentro de la red del cliente: hoy
 * se configura con un proveedor externo (ej. Gmail con clave de aplicacion), lo que exige que la
 * maquina cliente tenga salida a internet en el puerto SMTP configurado. Ver AGENTS.md.</p>
 *
 * <p>La configuracion se resuelve con el mismo patron ya usado por {@link TotpSecretCipher} y
 * {@code SdrercAppConnection}: propiedad en {@code config/sdrerc-app.properties} o variable de
 * entorno, nunca hardcodeada en el fuente. La contraseña SMTP nunca se guarda en claro en el
 * repositorio.</p>
 */

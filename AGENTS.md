...
- `Asignacion`: el asignador puede corregir datos registrales antes de asignar solo para expedientes `REGISTRO / REGISTRADO`; el tipo de documento solo puede cambiarse a `Carta` y el procedimiento registral solo a `Reconsideracion` o `Apelacion`, validando en UI y DAO sin cambiar el numero de expediente.

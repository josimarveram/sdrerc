...
- En carga diaria, `CANAL RECEPCIÓN` puede venir informado desde plantilla con lista desplegable; si queda vacio, se aplica la derivacion automatica existente. La referencia externa `N° EXPEDIENTE DIGITAL SITD` debe guardarse en `EXPEDIENTE_SOLICITUD.NUMERO_EXPEDIENTE_DIGITAL_SITD` y extender las busquedas de bandejas sin confundirse con numero de expediente SDRERC, tramite web ni numero de documento.

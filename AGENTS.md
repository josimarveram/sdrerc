...
- Pendiente antes de poder usarlo de punta a punta: (1) autorizar y ejecutar el script 82; (2) completar `mail.smtp.user`/`mail.smtp.password` (cuenta remitente + clave de aplicacion) en `config/sdrerc-app.properties` o via variables de entorno; (3) probar un login real con un usuario que tenga `correo` cargado.

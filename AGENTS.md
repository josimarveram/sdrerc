...
### Expediente ya asociado usado como "principal" de una nueva asociacion (SDRERC-EXP-2026-000179, 20/07/2026)

- Reporte del usuario: expediente principal recibido en Analisis; desde Asignacion se asociaron 2 solicitudes duplicadas al mismo principal, en 2 acciones separadas. Resultado esperado: 1 principal con 2 asociados. Resultado real: Asignacion mostraba solo 1 asociado, y Analisis mostraba 2 filas "principales" (mismo numero SDRERC-EXP-2026-000179 repetido), una con 1 asociado y otra con otro asociado distinto.
- Diagnostico contra BD (autorizado por el usuario): el expediente numero SDRERC-EXP-2026-000179 corresponde a 3 filas EXPEDIENTE distintas (202=ANALISIS/RECIBIDO_POR_ABOGADO, el principal real; 203=ASIGNACION/ASIGNADO; 204=REGISTRO/REGISTRADO), todas con el mismo numero porque el duplicado hereda el numero del principal (regla ya documentada). `EXPEDIENTE_RELACION` tenia: id=41 (principal=202,relacionado=203,activo=0, version anterior), id=43 (principal=202,relacionado=203,activo=1, correcta), e id=42 (principal=**203**,relacionado=204,activo=1) — esta ultima con el error: uso 203 (que ya era un asociado de 202) como si fuera un principal nuevo, en vez de resolver hacia el principal real 202. Esto partio el grupo en 2 arboles de relacion distintos con la misma raiz de numero pero id_expediente_principal diferente, lo que explica ambos sintomas (Asignacion agrupa por relaciones directas de un solo id_expediente_principal a la vez; Analisis termina mostrando 203 tanto como asociado de 202 como principal de 204).
- Causa raiz en codigo: `ExpedienteRelacionadoDAO.asociarRelacionados`/`resolverOrientacionRelacion`/`elegirPrincipalAsociacion` (UI, `JPanelAsignacionV2`) nunca verificaban si el expediente que se iba a usar como "principal" de una nueva asociacion ya era el "relacionado" (asociado) de otro principal existente en una relacion activa. Como los duplicados heredan el numero del principal, el criterio existente ("primero el que tiene numero SDRERC; si empatan, el mas antiguo") ya no alcanza para desempatar una vez que el duplicado hereda numero — hacia falta resolver primero contra el grafo de `EXPEDIENTE_RELACION` existente, no solo contra los 2 candidatos de la accion en curso.
- Fix: nuevo metodo privado `ExpedienteRelacionadoDAO.resolverPrincipalCanonico(conn, idExpediente)` que, antes de cualquier otra validacion dentro de `asociarRelacionados(...)`, sigue la cadena de relaciones activas (`id_expediente_relacionado = idExpediente` -> toma `id_expediente_principal` de esa fila, repite hasta no encontrar mas, tope de 20 saltos por seguridad) y sustituye el `idExpedientePrincipal` recibido por su raiz real antes de insertar la nueva relacion. Aplica tanto si la llamada viene de Asignacion (`JPanelAsignacionV2`) como de Registro (`JPanelBandejaExpedientesNueva`), porque ambas comparten el mismo `ExpedienteRelacionadoService`/`ExpedienteRelacionadoDAO`.
- Correccion puntual de datos (autorizada explicitamente por el usuario): `db/sdrerc_app/scripts/78_correccion_relacion_principal_incorrecto_179.sql` (ejecutado) corrige unicamente la fila `id_expediente_relacion=42` de `SDRERC-EXP-2026-000179`, cambiando `id_expediente_principal` de 203 a 202. Verificado despues del UPDATE: las 2 relaciones activas (42 y 43) del expediente ya apuntan al mismo principal 202. No se toco ninguna otra fila ni expediente.

### Validacion de unicidad de "N° expediente SGD" en Registro manual, Edicion manual y Carga diaria (20/07/2026)

- Pedido del usuario: agregar validacion para que `N° expediente SGD` sea unico y no exista duplicado de ese valor en Carga diaria y en Registro/Edicion manual. Ya era obligatorio (no vacio); no existia ninguna validacion de unicidad.
- Alcance acordado con el usuario: la unicidad se valida solo contra expedientes `activo=1` y que NO sean un "relacionado" (asociado) dentro de una relacion activa de `EXPEDIENTE_RELACION` (para no chocar con el caso normal de duplicados asociados, que heredan datos del principal). En Carga diaria, un SGD duplicado bloquea esa fila especifica (no se registra, el resto del lote si).
- Nuevo `ExpedienteRegistroDAO.detectarDuplicadoPorNumeroExpedienteSgd(String, Long idExpedienteExcluir)` (uso individual: Registro/Edicion manual) y `detectarDuplicadosPorNumeroSgdContraBase(List<CargaDiariaPreviewDTO>)` (uso batch: Carga diaria), ambos sobre el metodo privado compartido `buscarRegistroPorNumeroExpedienteSgd(conn, numeroExpedienteSgd, idExpedienteExcluir)`: `SELECT e.id_expediente, e.numero_expediente FROM expediente e JOIN expediente_solicitud es ON es.id_expediente=e.id_expediente AND es.activo=1 WHERE e.activo=1 AND UPPER(TRIM(es.numero_expediente_sgd))=? AND NOT EXISTS (SELECT 1 FROM expediente_relacion r WHERE r.activo=1 AND r.id_expediente_relacionado=e.id_expediente) [AND e.id_expediente <> ?]`. Reutiliza la clase interna `DuplicadoRegistro` ya usada por la deteccion de duplicados por acta+titular.
- `RegistroManualExpedienteDTO` (base de `ExpedienteEdicionManualDTO`) gano 2 campos nuevos: `numeroExpedienteSgdDuplicado` (boolean) y `motivoNumeroExpedienteSgdDuplicado` (String), para distinguir esta violacion — que debe **bloquear** — de las advertencias existentes (duplicado por acta+titular, procedimiento sin numero, etc.) que son solo informativas y no impiden registrar.
- `RegistroManualExpedienteService.validarConDuplicados(...)` (Registro manual) consulta el DAO y, si hay coincidencia, agrega el mensaje a la lista Y marca el flag bloqueante en el DTO. `registrar(...)` ahora revisa ese flag despues de validar y lanza `IllegalArgumentException` (bloqueo real del lado servidor) antes de llegar a `expedienteRegistroDAO.registrarManual(...)` si el SGD ya esta en uso; antes de este cambio, `registrar(...)` NUNCA bloqueaba nada (solo concatenaba advertencias en `observacionesGenerales` y persistia igual, mismo patron que el duplicado por acta+titular, que si debe poder guardarse sin numero).
- `ExpedienteEdicionManualService` ahora recibe `ExpedienteRegistroDAO` (constructor de 3 argumentos; el de 0 argumentos sigue existiendo para los 2 callers reales, que no cambiaron). `validar(ExpedienteEdicionManualDTO)` paso a `throws SQLException` (unico caller ya estaba dentro de un `SwingWorker.doInBackground()` que ya declaraba `throws Exception`) y agrega el mismo chequeo de unicidad, excluyendo `dto.getIdExpediente()` para no comparar el expediente contra si mismo. Como `guardar(...)`/`guardarDesdeAnalisis(...)` ya relanzaban cualquier error de `validar(...)` como `IllegalArgumentException` antes de tocar el DAO de persistencia, el bloqueo del lado servidor para Edicion manual salio gratis con este cambio.
- UI (`JPanelRegistroManualRecepcionV2.aplicarValidacion`): antes, el boton "Registrar expediente"/"Guardar cambios" solo se deshabilitaba con errores presentes cuando `modoEdicion()` era true (Registro manual nuevo dejaba el boton habilitado pese a tener errores en pantalla, patron pre-existente para permitir registrar duplicados por acta+titular sin numero). Ahora la condicion agrega `&& !dto.isNumeroExpedienteSgdDuplicado()`, aplicando el bloqueo tambien al registro nuevo; el mensaje de estado muestra el motivo especifico cuando aplica.
- `CargaDiariaReglasService.validar(...)`: se agrego deteccion de SGD repetido **dentro del mismo Excel** (mismo patron de mapa de conteo + "primera fila" que ya existia para acta+titular) y **contra BD** (`detectarDuplicadosPorNumeroSgdContraBase`, llamada una vez por lote antes del loop, igual que las demas detecciones contra base). A diferencia de TODAS las demas validaciones "obligatorio"/observacion de este metodo (que nunca bloqueaban: `listoParaRegistrar` quedaba `true` sin importar cuantos errores hubiera, patron pre-existente confirmado antes de este cambio), el SGD duplicado es la primera condicion que realmente bloquea: `item.setListoParaRegistrar(false)` y `item.setEstadoValidacion("Error")` (estado que `CargaDiariaResumenDTO.desde(...)` ya sabia sumar a `conErrores` pero que ningun flujo generaba hasta ahora), con un `continue` que salta el resto del calculo de esa fila (no se le asigna numero preliminar, para no gastar un correlativo en una fila que no se va a registrar). El resto del lote sigue el flujo normal y si se registra al confirmar.
- No se agrego restriccion `UNIQUE` en `EXPEDIENTE_SOLICITUD.numero_expediente_sgd` a nivel Oracle (solo existe el indice no unico `ix_exp_sol_sgd`, creado por `23_patch_rename_expediente_solicitud_sgd.sql`). La validacion es 100% de aplicacion; si se quiere el refuerzo de integridad a nivel BD, es un paso aparte pendiente de autorizacion (requiere primero revisar si ya existen SGD duplicados historicos, porque un `ADD CONSTRAINT UNIQUE` fallaria si los hay).

### Mejora de UX: asociacion de duplicados, confusion sobre cual expediente seleccionar (20/07/2026)

- Pedido del usuario: en Asignacion y en Registro (Asociar duplicados), no quedaba claro si se debia seleccionar el expediente principal (el que ya tiene numero) o el potencial duplicado (sin numero) para asociarlos; pidio que la interfaz fuera mas amigable e intuitiva, y que el criterio fuera: el principal es el que ya tiene numero, los demas (sin numero) son los que se asocian a el.
- Decision de UX confirmada con el usuario: si entre las coincidencias marcadas/detectadas hay un solo expediente con numero, se asume automaticamente como principal (el usuario solo marca los duplicados sin numero). Si hay mas de uno con numero entre los marcados, es ambiguo y no se decide solo: se bloquea la accion con un mensaje claro pidiendo dejar marcado unicamente el principal real.
- Causa raiz de la confusion (no era un bug de datos; el backend ya reorientaba correctamente via `resolverOrientacionRelacion`/`resolverPrincipalCanonico`, ver entrada anterior sobre SDRERC-EXP-2026-000179): la UI trataba ciegamente como "principal" al expediente que el usuario tenia enfocado/seleccionado en la bandeja, sin verificar si realmente tenia numero, y sin indicar visualmente cual de las coincidencias en la tabla de candidatos ya tenia numero.
- `JPanelAsignacionV2.java` (lengueta "Asociar"):
  - Nuevo helper `resolverPrincipalParaFoco(AsignacionExpedienteDTO foco, List<Long> idsMarcados)` (y su resultado `ResolucionAsociacionFoco`): si el expediente enfocado no tiene numero pero entre las solicitudes marcadas en "Solicitudes asociadas" hay exactamente una CON numero, invierte la asociacion (esa marcada pasa a ser el principal real, y el expediente enfocado se agrega como uno de los relacionados). Si hay mas de una marcada con numero, retorna `ambiguo=true`.
  - `obtenerSeleccionAsociacionRapida()` (flujo de checkboxes en la grilla principal, 2+ marcados) ahora tambien detecta ambiguedad: si mas de un marcado tiene numero, no elige "el mas antiguo" en silencio (comportamiento previo), sino que marca `ambiguo=true` y `asociarRelacionadosRapido()` bloquea la accion con mensaje explicativo.
  - `asociarRelacionadosRapido()` usa el resultado resuelto (no el foco/seleccion cruda) tanto para el `idPrincipal` que se envia al backend como para el texto "Expediente principal destino" del dialogo de confirmacion, evitando que la etiqueta mienta sobre quien terminara siendo el principal real.
  - Actualizacion reactiva: al marcar/desmarcar checkboxes en "Solicitudes asociadas" (`actualizarBotonAsociarPorSeleccionTabla`), la etiqueta "Expediente principal" (`lblExpedientePrincipalAsociacion`, ahora en `<html><b>...</b></html>` para resaltar el numero) y el texto/estado del boton se recalculan en vivo con el mismo resolver, sin esperar a que el usuario confirme la accion.
  - Columna "N° expediente" de la tabla de candidatos (`documentosRelacionadosTable`) ahora muestra literalmente "Sin número (potencial duplicado)" en vez de "-" cuando el candidato no tiene numero, y el panel agrego un texto de ayuda fijo: "Marque el/los expedientes sin número (potenciales duplicados) para asociarlos al expediente principal, que es el que ya tiene número."
- `JPanelBandejaExpedientesNueva.java` (panel "Asociar duplicados" de Registro/Recepcion, clase interna `JPanelAsociarDuplicadosRecepcionV2`): mismo patron aplicado a su unico flujo (aqui no hay seleccion multiple por checkbox en la grilla principal, solo seleccion de una fila como foco). Nuevo `resolverPrincipal(List<Long> idsSeleccionados)` (con resultado `ResolucionPrincipalDuplicados`, clase a nivel de la clase externa porque una clase interna no-estatica no puede declarar miembros `static` en Java 8) aplicado en `asociarSeleccionados()`, `asociarRapido()` y la actualizacion reactiva del label al marcar/desmarcar checkboxes en la tabla "Coincidencias por N° de acta y titular" (`actualizarEtiquetaPrincipalPorSeleccion`, enganchada a un `TableModelListener` nuevo sobre `modeloDuplicados`). Mismo texto "Sin número (potencial duplicado)" en la columna de numero y mismo texto de ayuda agregado bajo el titulo de la tabla de candidatos.
- No se toco el backend de asociacion (`ExpedienteRelacionadoService`/`ExpedienteRelacionadoDAO`, incluido `resolverPrincipalCanonico` de la entrada anterior): esta mejora es exclusivamente de UI/UX, para que la interfaz sea transparente sobre lo que el backend ya hace correctamente.

### Propagacion de etapa/estado del expediente principal a sus asociados (20/07/2026)

- Pedido del usuario: "desde la asignacion hasta el ultimo cambio de estado del expediente principal deberia afectar ese mismo cambio a todos los documentos asociados por el mismo nro de expediente." Confirmado con el usuario: aplica a TODO el flujo desde Asignacion en adelante (Asignacion, Analisis, Verificacion, Ejecucion, Notificacion, Cierre), reemplazando los pasos manuales existentes (ej. "Recibir documento asociado" en Analisis).
- Antes de este cambio, cada expediente asociado (duplicado confirmado, EXPEDIENTE_RELACION activa, tipo_relacion=DOCUMENTO_DUPLICADO_ASOCIADO) mantenia su propia etapa/estado independiente del principal; solo se sincronizaba equipo/abogado/fecha_vencimiento al momento de asociar, y la recepcion en Analisis requeria una accion manual por asociado.
- Nuevo `ExpedienteEstadoPropagacionDAO` (paquete `infrastructure.sdrercapp.dao`, package-private, sin estado, mismo patron que `VisibilidadBandejaSql`): `propagarEstadoAAsociados(Connection conn, Long idExpedientePrincipal, Long idEtapaDestino, Long idEstadoDestino, Long idUsuarioModificador)` consulta los asociados activos del principal (`EXPEDIENTE_RELACION` con `tipo_relacion='DOCUMENTO_DUPLICADO_ASOCIADO'` y `activo=1`), y para cada uno cuya etapa/estado actual sea distinto del destino, actualiza `EXPEDIENTE.id_etapa_actual`/`id_estado_actual` (mismo `UPDATE` minimo que ya usaba cada DAO, sin tocar responsable/equipo) e inserta una fila en `EXPEDIENTE_HISTORIAL` con el nuevo tipo de movimiento `PROPAGACION_ESTADO_ASOCIADO` (sembrado por `db/sdrerc_app/scripts/79_movimiento_propagacion_estado_asociado.sql`, ya ejecutado), dejando trazabilidad de que el cambio vino del principal.
- Punto de insercion elegido (no los ~29 sitios de llamada, sino el metodo compartido de bajo nivel): se agrego una sola linea `ExpedienteEstadoPropagacionDAO.propagarEstadoAAsociados(...)` al final del metodo privado `actualizarExpediente`/`actualizarExpedienteConDestino` de cada uno de estos 9 DAOs (cada uno tenia su propia copia duplicada de ese metodo, no hay clase base compartida): `AsignacionExpedienteDAO`, `AnalisisExpedienteDAO`, `VerificacionExpedienteDAO` (los 2 metodos: `actualizarExpediente` y `actualizarExpedienteConDestino`), `EjecucionExpedienteDAO`, `NotificacionExpedienteDAO`, `FirmaEmisionExpedienteDAO` (usado internamente por Verificacion, aunque no tiene menu lateral propio), `PublicacionExpedienteDAO`, `CierreArchivoDAO`, `ExpedienteDigitalDAO`. Como es el mismo `Connection`/transaccion que el UPDATE del principal, la propagacion queda atomica con la transicion que la origino.
- Caso especial `DocumentoAnalisisDAO`: `asignarNotificacionMultiple`/`reasignarNotificacion` y `registrarResultadoValidacion` cambian `EXPEDIENTE.id_estado_actual` (sin cambiar etapa, siempre dentro de NOTIFICACION) con SQL inline propio, no via un `actualizarExpediente` compartido; se agrego la llamada a `propagarEstadoAAsociados` en esos 2 puntos tambien, resolviendo `id_etapa_actual` vigente del expediente (no cambia) para poder llamar al mismo helper.
- `AnalisisExpedienteDAO.recibirExpediente`: se elimino el bucle manual que recibia cada asociado uno por uno (`listarIdsAsociadosConfirmados` + `bloquearExpediente` + `recibirExpedienteAsignado` por cada uno), porque ahora queda cubierto automaticamente por la propagacion que dispara `actualizarExpediente` al recibir el principal; se conserva `listarIdsAsociadosConfirmados(...).size()` solo para el mensaje "N documento(s) asociado(s) tambien fueron recibidos.".
- `AnalisisExpedienteDAO.recibirDocumentoAsociado` (el metodo/boton "Recibir" agregado en la tarea anterior de esta sesion) **no se elimino**: queda como fallback defensivo. En el flujo normal ya no hace falta usarlo (el asociado ya habra avanzado junto con el principal por la propagacion), y el boton correspondiente en `JPanelAnalisisV2` ya queda deshabilitado/mostrando "Recibido" automaticamente en cuanto el asociado cambia de estado (la UI lee el estado real desde BD, no requirio cambios).
- No se propago responsable/equipo, solo etapa/estado: la sincronizacion de equipo/abogado del asociado al momento de asociarse (`sincronizarAsignacionDesdePrincipal`, ya existente en `ExpedienteRelacionadoDAO.asociarRelacionados`) sigue siendo el unico mecanismo para eso; mezclarlo con esta propagacion habria sido un cambio de alcance mayor no pedido.
- `db/sdrerc_app/scripts/79_movimiento_propagacion_estado_asociado.sql` (autorizado y ejecutado): siembra el codigo `PROPAGACION_ESTADO_ASOCIADO` en `TIPO_MOVIMIENTO`, idempotente (`WHERE NOT EXISTS`).

### Fase 2 de Grupo Familiar: panel "Asociar" con ID unico de grupo, en Registro y Asignacion (20/07/2026)

- Pedido del usuario: configurar un panel de "Asociar Grupo Familiar" en Registro/Recepcion (ya existia uno muy simple, "Registrar G.F.") y en Asignacion (no existia ninguno), tomando como referencia el panel "Asociar duplicados"/"Asociar" pero SIN la logica de principal/duplicado: solo se marca "Si" en Grupo Familiar de la persona titular. Ademas pidio una funcionalidad nueva: un ID unico de grupo familiar para saber que otras personas pertenecen al mismo grupo y que expedientes tienen.
- Decisiones de diseno confirmadas con el usuario: (1) el ID de grupo familiar es puramente interno, sin codigo visible tipo SDRERC-EXP; (2) el vinculo se hace a nivel PERSONA, no por expediente, para que si la misma persona vuelve a ser titular de otro expediente en el futuro herede el grupo automaticamente sin tener que re-asociarla.
- Contexto previo ya documentado en `docs/arquitectura_app/DIAGNOSTICO_GRUPO_FAMILIAR_REGISTRO_RECEPCION.md` (fecha 2026-06-20): la Fase 1 (flag booleano simple `EXPEDIENTE_SOLICITUD.grupo_familiar`, sembrada por `27_grupo_familiar_fase1.sql`) ya estaba implementada. Esta tarea implementa la Fase 2 propuesta en ese diagnostico (confirmar relacion, mostrar integrantes), simplificada respecto a la Fase 3 completa (no se crearon tablas `GRUPO_FAMILIAR_EXPEDIENTE`/`PARENTESCO`; el vinculo persona-grupo es directo via `PERSONA.id_grupo_familiar`, sin tabla intermedia, porque una persona pertenece a lo sumo a un grupo).
- BD (`db/sdrerc_app/scripts/80_grupo_familiar_fase2.sql`, autorizado y ejecutado): tabla ancla `GRUPO_FAMILIAR` (solo id + auditoria) y columna `PERSONA.id_grupo_familiar` (nullable, FK a `GRUPO_FAMILIAR`, indice `ix_persona_grupo_familiar`). No se reutilizo `EXPEDIENTE_RELACION` (esa tabla hereda numero de expediente/equipo/abogado para duplicados; grupo familiar no debe heredar nada de eso, son conceptos distintos, tal como advertia el diagnostico). Tambien siembra el tipo de movimiento `ASOCIACION_GRUPO_FAMILIAR` para el historial. Nota tecnica: los bloques PL/SQL del script requieren el delimitador `/` en linea propia despues de cada `END;` para que SQL*Plus los ejecute (sin el, `sqlplus` se queda esperando indefinidamente sin ejecutar nada); si se edita este script a futuro, no quitar esos `/`.
- Nuevo `GrupoFamiliarDAO` (paquete `infrastructure.sdrercapp.dao`): `listarPosiblesIntegrantes(idExpediente)` detecta candidatos consultando expedientes activos cuyo titular comparte la misma "clave de apellidos" (`GrupoFamiliarHeuristicaService.claveApellidosTitular`, la misma heuristica que ya usaba Carga diaria para detectar coincidencias dentro del Excel) contra un tope de 3000 filas, filtrando en Java (mismo patron ya usado en `ExpedienteRegistroDAO.buscarClavesGrupoFamiliarExistentes`, no hay forma de calcular la clave en SQL). `asociarGrupoFamiliar(idExpedientePrincipal, idsExpedientesCandidatos, idUsuario)` resuelve el `id_persona` titular de cada expediente, bloquea (`FOR UPDATE`) el grupo actual de cada persona involucrada: si ninguna tiene grupo, crea uno nuevo (`GRUPO_FAMILIAR`); si exactamente una ya tiene grupo, las demas se unen a ese; si hay mas de un grupo distinto entre los seleccionados, bloquea con excepcion (mismo patron de ambiguedad que la resolucion de principal de duplicados de la tarea anterior). Tambien marca `EXPEDIENTE_SOLICITUD.grupo_familiar=1` (criterio `CONFIRMADO_ASIGNACION`, mismo valor que ya proponia el diagnostico) para mantener sincronizada la Fase 1, e inserta `EXPEDIENTE_HISTORIAL` por cada expediente afectado. `listarIntegrantesGrupoFamiliar(idExpediente)` devuelve todas las personas del mismo grupo que el titular del expediente dado, con su propio expediente/etapa/estado/abogado — esto resuelve el pedido de "saber que expedientes tienen las otras personas".
- Nuevo `GrupoFamiliarService` (capa `application.sdrercapp`) envolviendo el DAO, mismo patron que `ExpedienteRelacionadoService`.
- 3 DTOs nuevos en `domain.dto.sdrercapp`: `GrupoFamiliarCandidatoDTO`, `GrupoFamiliarIntegranteDTO`, `GrupoFamiliarResultadoDTO` (este ultimo mismo shape que `ExpedienteRelacionResultadoDTO`).
- UI Registro (`JPanelBandejaExpedientesNueva.java`, clase interna `JPanelRegistrarGrupoFamiliarV2`, tab "Grupo Familiar" en el card layout de recepcion ya existente `PANEL_RECEPCION_CARD_GF`): rediseñado con el mismo patron visual que "Asociar duplicados" — seccion "Posibles integrantes" (tabla de candidatos por apellidos con checkbox, texto de ayuda explicando que solo se marca "Si", no se asocian expedientes), boton "Asociar al grupo familiar", y seccion de solo lectura "Grupo familiar actual" mostrando los integrantes ya confirmados. Se conservo ademas el flujo previo de seleccion multiple por checkbox en la grilla principal (`grupoFamiliarSeleccionados`), pero ahora el boton "Asociar seleccionados" llama a `grupoFamiliarService.asociarGrupoFamiliar(...)` (agrupa de verdad con un ID compartido) en vez del `GrupoFamiliarRegistroService` anterior (que solo marcaba el flag de cada expediente de forma independiente, sin vincularlos). El metodo publico `registrarGrupoFamiliarSeleccionados()` (todavia usado por el panel duplicado en `JPanelRegistroRecepcionV2`, ver nota abajo) se dejo intacto usando el `GrupoFamiliarRegistroService` original.
- UI Asignacion (`JPanelAsignacionV2.java`): lengueta nueva "Grupo Familiar" (`TAB_PANEL_GRUPO_FAMILIAR`, color verde `new Color(35,138,94)`), agregada al set de lenguetas visibles en `MODO_PANEL_ASIGNACION` junto a Datos/Asignacion/Asociar (no aparece en `MODO_PANEL_RESPUESTA`, que sigue mostrando solo Datos/Respuesta). Panel nuevo `crearPanelGrupoFamiliar()` con la misma estructura que el de Registro (candidatos + boton + grupo actual). Se carga automaticamente al seleccionar un expediente principal (junto a `cargarDocumentosRelacionadosPanel`) y al activar la lengueta.
- Advertencia sobre codigo preexistente NO tocado por esta tarea: `JPanelRegistroRecepcionV2.java` tiene su PROPIA clase interna duplicada `JPanelRegistrarGrupoFamiliarV2` (copy-paste de la de `JPanelBandejaExpedientesNueva`, ya existia antes de esta tarea) que sigue usando el flujo simple de solo marcar flags (via `bandejaRegistro.registrarGrupoFamiliarSeleccionados()`). No se determino con certeza si ese panel duplicado es el que efectivamente se le muestra al usuario en el modulo "Registro/Recepcion" (podria coexistir con el panel interno de `JPanelBandejaExpedientesNueva`, que SI se actualizo). Si en uso real resulta que ese es el panel visible y no el actualizado, hace falta revisarlo en una tarea aparte.

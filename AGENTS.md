...
- Segundo seguimiento (mismo dia): el usuario aclaro que ademas queria, en desarrollo local, poder ver la version del **cliente instalado** en la misma maquina (no solo la version del `pom.xml`). Se agrego un tercer nivel de resolucion, intercalado entre los dos anteriores: `VersionInfoReader` ahora tambien intenta leer `C:\SDRERC_CLIENTE\app\version-local.json` (ruta fija documentada como destino del launcher) antes de caer al respaldo de `pom.xml`. Orden final de `leerEtiquetaVersion()`: (1) `version-local.json` de la copia que esta corriendo ahora mismo -> `"Versión X · fecha"`; (2) si no existe, `version-local.json` del cliente instalado en esta maquina -> `"Versión cliente instalada X · fecha"`; (3) si tampoco existe, `pom.xml` -> `"Versión X (desarrollo local)"`. Se extrajo `formatearEtiquetaDesdeArchivo(File, String prefijo)` como helper comun para (1) y (2), que antes estaban duplicados. Verificado manualmente creando y luego eliminando un `C:\SDRERC_CLIENTE\app\version-local.json` de prueba: con el archivo presente se obtuvo `"Versión cliente instalada 1.0.5 · 20/07/2026"`; sin el archivo, volvio a `"Versión 1.0.0 (desarrollo local)"`.

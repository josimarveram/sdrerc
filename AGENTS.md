...
- `Asignacion`: la lengueta `Asociar` permite reasignar un expediente ya asignado a un abogado/equipo distinto. La reasignacion exige una hoja de envio NUEVA (no reutiliza ni sobrescribe la anterior), desactiva (`activa=0`) la fila vigente en `EXPEDIENTE_ASIGNACION` sin eliminarla, inserta una nueva fila con `es_reasignacion_excepcional=1` y actualiza el responsable/equipo del expediente. El panel `Asociar` muestra un historial de asignaciones/reasignaciones (activas e historicas) por expediente.

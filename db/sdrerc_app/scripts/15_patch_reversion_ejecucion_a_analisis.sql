...
-- No lo ejecutes en producción. Conéctate como SDRERC_APP, valida backup primero y ejecútalo solo si quieres limpiar data transaccional de expedientes. No toca catálogos,
-- usuarios, roles ni equipos.

  -- 1) Vista previa
  SELECT 'EXPEDIENTE' tabla, COUNT(*) cantidad FROM expediente
  ...
  UNION ALL SELECT 'EXPEDIENTE_DOCUMENTO', COUNT(*) FROM expediente_documento
  UNION ALL SELECT 'EXPEDIENTE_EVALUACION', COUNT(*) FROM expediente_evaluacion
  UNION ALL SELECT 'EXPEDIENTE_HISTORIAL', COUNT(*) FROM expediente_historial;

  -- 2) Purga transaccional de expedientes y tablas asociadas
  DECLARE
    v_personas SYS.ODCINUMBERLIST;
  BEGIN
    SELECT DISTINCT id_persona
    BULK COLLECT INTO v_personas
    FROM (
      SELECT id_persona
      FROM expediente_persona
      WHERE id_persona IS NOT NULL
      UNION
      SELECT id_persona_solicitante
      FROM expediente_solicitud
      WHERE id_persona_solicitante IS NOT NULL
    );

    DELETE FROM expediente_cargo_acuse;
    DELETE FROM expediente_publicacion;
    DELETE FROM expediente_notificacion;
    DELETE FROM expediente_derivacion_externa;
    DELETE FROM expediente_resolucion;
    DELETE FROM expediente_documento_analizado;
    DELETE FROM expediente_documento;
    DELETE FROM expediente_metrica_etapa;
    DELETE FROM expediente_alerta;
    DELETE FROM expediente_plazo;
    DELETE FROM expediente_digital;
    DELETE FROM expediente_observacion;
    DELETE FROM expediente_evaluacion;
    DELETE FROM expediente_asignacion;
    DELETE FROM expediente_relacion;
    DELETE FROM expediente_acta;
    DELETE FROM expediente_solicitud;
    DELETE FROM expediente_persona;
    DELETE FROM expediente_historial;
    DELETE FROM expediente;

    DELETE FROM persona
    WHERE id_persona IN (SELECT COLUMN_VALUE FROM TABLE(v_personas));

    DELETE FROM auditoria_evento
    WHERE UPPER(tabla_afectada) IN (
      'EXPEDIENTE',
  ...
  --Luego resetea los identity a 1:

  ALTER TABLE persona MODIFY id_persona GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente MODIFY id_expediente GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_solicitud MODIFY id_expediente_solicitud GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_persona MODIFY id_expediente_persona GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_acta MODIFY id_expediente_acta GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_relacion MODIFY id_expediente_relacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_asignacion MODIFY id_expediente_asignacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_documento MODIFY id_expediente_documento GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_documento_analizado MODIFY id_documento_analizado GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_evaluacion MODIFY id_expediente_evaluacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_observacion MODIFY id_expediente_observacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_resolucion MODIFY id_expediente_resolucion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_notificacion MODIFY id_expediente_notificacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_cargo_acuse MODIFY id_expediente_cargo_acuse GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_digital MODIFY id_expediente_digital GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_historial MODIFY id_expediente_historial GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_derivacion_externa MODIFY id_derivacion_externa GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_publicacion MODIFY id_expediente_publicacion GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_plazo MODIFY id_expediente_plazo GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_alerta MODIFY id_expediente_alerta GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE expediente_metrica_etapa MODIFY id_expediente_metrica_etapa GENERATED BY DEFAULT AS IDENTITY (START WITH 1);
  ALTER TABLE auditoria_evento MODIFY id_auditoria_evento GENERATED BY DEFAULT AS IDENTITY (START WITH 1);

  --Si Oracle no acepta el ALTER TABLE ... IDENTITY (START WITH 1), dime la versión exacta de Oracle y te paso la variante compatible.

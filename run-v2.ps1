...
$mainClass = "com.sdrerc.appv2.MainV2"
$mainClassFile = Join-Path $root "target\classes\com\sdrerc\appv2\MainV2.class"

if (-not (Test-Path $mainClassFile)) {
    $mvn = Get-Command mvn -ErrorAction SilentlyContinue
    if ($mvn) {
        Write-Host "Compilando SDRERC V2..."
        & $mvn.Source -f (Join-Path $root "pom.xml") clean compile
    }
    else {
        Write-Error "No existe target/classes para MainV2 y Maven no esta disponible en PATH. Compile primero con: mvn clean compile"
    }
}
...
Write-Host "Iniciando SDRERC V2..."

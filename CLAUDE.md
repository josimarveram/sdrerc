...
- Mini-panel "Emision": replica el diseno del panel `Verificar` de Verificacion, no solo la grilla. Arriba, bloque "Resultado de Supervision" con combo Aprobado/Observado (fijo en codigo, sin catalogo de BD, igual que el combo Resultado de Verificar); por defecto Aprobado, y se reinicia a Aprobado cada vez que cambia el documento enfocado. **Aprobado** (por defecto) muestra la grilla `Documentos a firmar` (`DocumentoFirmaNotificacionTreeGridPanelV2`): lista TODOS los documentos del expediente del documento enfocado (no solo el documento cuya fila esta seleccionada en la bandeja, que en Asignacion es por-documento no por-expediente), columnas `Tipo documento`/`Numero Documento`/`Estado documento`/`Fecha Emision`. Numero, Fecha y Estado documento son editables (Estado documento con combo acotado a Validado/Emitido), y solo en las filas elegibles para firma: `INTERMEDIO` siempre, `FINAL` solo si su estado es `VALIDADO`; las filas no elegibles se ven en gris con el icono Guardar deshabilitado y tooltip explicando el motivo. **Observado** oculta la grilla y muestra su propio bloque "Comentario" (obligatorio) + "Destino operativo" (Eq. Analisis/Eq. Ejecucion, con su propio combo de equipo/usuario) + boton "Devolver expediente", que reutiliza el mismo `derivarAsigNotifADestinoOperativo` ya usado por el "Destino operativo" del mini-panel Asignacion (representa al supervisor de Asignacion observando el documento antes de emitirlo, distinto de cuando el validador ya lo observo, pero mismo mecanismo/destino final).

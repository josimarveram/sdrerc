...
- Boton "Dashboard" en `MenuPrincipalV2`, en su propio bloque de menu "Dashboard y Reportes" (separado de "Inicio" y de "Administracion": es una vista analitica de alto nivel, no configuracion/catalogos), gateado por el permiso `MENU_DASHBOARD` (`94_permiso_menu_dashboard.sql`, campo `modulo='Dashboard y Reportes'` para que se agrupe igual en Administracion > Roles > "Permisos del rol", otorgado unicamente a `ADMIN_SISTEMA`, no ejecutado). `JPanelDashboardV2.tieneAcceso()`/`DashboardService.tieneAcceso()` (via `SessionContext.hasRole("ADMIN_SISTEMA")`) es una segunda capa de defensa: si alguien igual instancia el panel sin el permiso de menu, muestra un mensaje de acceso restringido en vez de lanzar una excepcion o mostrar datos.
...
- Carga diaria y registro manual detectan duplicidad por `numero de acta + titular completo`, con comparacion de titular insensible a tildes/diacriticos (`GrupoFamiliarHeuristicaService.coincideExactamente`, no `UPPER(TRIM(...))` SQL puro). Registro manual y Edicion manual (comparten `ExpedienteRegistroDAO.buscarRegistroPorActaYTitular`) aplican ademas una 2da condicional de respaldo (07/08/2026): si ningun titular con esa acta coincide exactamente, pero existe al menos una solicitud activa con el mismo numero de acta (titular distinto: letra cambiada, tilde de mas, orden nombres/apellidos invertido), tambien se marca "Potencial duplicado" solo por coincidir el acta. Sin acta igual y sin titular igual, no hay duplicado. Carga diaria hereda el mismo comportamiento en su cruce contra BD ya registrada (mismo metodo compartido); su deteccion de duplicados dentro del propio archivo Excel cargado sigue exigiendo acta+titular exacto (sin el fallback solo-por-acta), sin cambios.
...
- Al eliminar (baja logica) un registro desde Recepcion (`JPanelBandejaExpedientesNueva`) o Asignacion (`JPanelAsignacionV2`) — ambas llaman al mismo `ExpedienteEdicionManualDAO.eliminar` —, las alertas `Potencial duplicado`/`Posible Grupo Familiar` de las solicitudes relacionadas (mismo numero de acta, o mismos apellidos del titular segun corresponda) se limpian solo si quedaban exactamente 2 solicitudes relacionadas (la eliminada + 1: la sobreviviente ya no tiene con quien ser duplicado/familiar); si eran 3 o mas, las demas conservan su alerta activa y solo se limpia la de la solicitud eliminada. Si el titular del registro eliminado pertenecia a un grupo familiar YA CONFIRMADO (`PERSONA.id_grupo_familiar`), se le retira de ese grupo (07/08/2026, pedido explicito del usuario).
...
- La bandeja solo muestra documentos cuyo `estado_final_notificacion_codigo` (mismo calculo de 4 estados que Bandeja Notificacion/Bandeja Publicacion: `POR_NOTIFICAR`/`PENDIENTE`/`ATENDIDO`/`POR_PUBLICAR`) sea `ATENDIDO` (07/08/2026, pedido explicito del usuario): el ciudadano ya fue ubicado en algun intento de notificacion, o el documento ya quedo `Publicado` en la Bandeja Publicacion de Notificacion. Ademas, deja de mostrarse en cuanto el expediente ya volvio a etapa `ANALISIS` (08/08/2026, pedido explicito del usuario: "ya no deberia aparecer en el listado... debido a que ya fue derivado a analisis"), sin importar el motivo de fecha (Fecha Acuse/Publ. Notif./Publ. Edicto); el seguimiento de ahi en adelante es responsabilidad de la Bandeja Analisis (KPI "Derivado"). Filtro aplicado directamente en SQL (`DocumentoAnalisisDAO.listarCartasRespuestaPendientes`), no en memoria.
- Columnas de fecha, en este orden (07/08/2026, pedido explicito del usuario): `Fecha Acuse`, `Fecha Publ. Notif.`, `Fecha Publ. Edicto`. Las 2 ultimas de solo lectura en esta bandeja: `Fecha Publ. Notif.` (de `EXPEDIENTE_NOTIFICACION` con `tipo_notificacion='PUBLICACION'` y `estado_notificacion='EXITOSA'`, el mismo 3er intento de la Bandeja Publicacion de Notificacion; vacia si el documento aun no fue publicado ahi) y `Fecha Publ. Edicto` (de `EXPEDIENTE_PUBLICACION`, el modulo standalone "Publicacion" — legal/edicto de la carta, no confundir con notificacion; hoy solo se llena para Carta Edicto).
- Columna `Estado` (07/08/2026, pedido explicito del usuario, reemplaza una version anterior basada en `confirmacion_respuesta`): `Edicto Publicado` una vez que se registra `Fecha Publ. Edicto` (hoy solo posible en Carta Edicto); `Pendiente de Respuesta` en cualquier otro caso (incluye Carta Edicto antes de publicar el edicto, y el resto de cartas intermedias — Carta Falta Sustento, Carta Precisar Pretension, Carta Indagatorio — que nunca llegan a tener `Fecha Publ. Edicto`). No existe rama `Derivado` en esta columna (08/08/2026, revertido el mismo dia que se agrego): un expediente ya derivado a Analisis ya no aparece en absoluto en esta bandeja (ver filtro de arriba), asi que nunca llega a mostrarse aqui con ningun estado.
- El boton "Registrar asignacion" de esta bandeja (lengueta "Cartas de respuesta" y su variante en el panel de "Documentos") deriva el expediente de vuelta a Analisis con estado `ANALISIS/DERIVADO` (08/08/2026, pedido explicito del usuario), no `ANALISIS/OBSERVADO` como antes: es una carta intermedia ya respondida por el ciudadano que el abogado debe continuar trabajando, no una observacion/rechazo real. `AsignacionExpedienteDAO.reasignarDesdeCartaRespuesta` gano un overload con `codigoEstadoDestino` (el overload de 5 argumentos historico sigue apuntando a `OBSERVADO` por compatibilidad); ambos botones de `JPanelAsignacionV2` (Cartas de respuesta y panel de Documentos) pasan `"DERIVADO"` explicitamente. El camino distinto — cuando el supervisor de Notificacion observa un documento y lo deriva a Analisis desde "Registrar Supervision" (`JPanelNotificacionV2.derivarAsigNotifADestinoOperativo`) — sigue usando `OBSERVADO` sin cambios: esa si es una observacion real que exige correccion. Estado nuevo `ANALISIS/DERIVADO` sembrado por `96_estado_derivado_analisis.sql` (no ejecutado), junto con las transiciones `DEVOLUCION_A_ANALISIS` (desde `NOTIFICACION/EN_NOTIFICACION` y `NOTIFICACION/NOTIFICADO`, mismo patron del script 91 pero destino `DERIVADO`) y `REGISTRO_RESULTADO_ANALISIS` (5 destinos desde `DERIVADO`, replica exacta del set ya sembrado para origen `OBSERVADO` en el script 20) necesarias para que el abogado pueda seguir operando el expediente en Analisis.
- "Registrar Asignación" de esta lengueta (`btnRegistrarAsignacionCarta`) exige que la carta intermedia seleccionada arriba ya tenga una carta de respuesta agregada en la grilla inferior (documento hijo, `nivel=1`, `id_documento_padre` = id del documento intermedio) antes de habilitarse (09/08/2026, pedido explicito del usuario: "NO debería permitir registrar asignación hacia análisis si no se ha agregado la carta de respuesta"). `cargarCartasRespuestaPorDocumento` (dispara tanto al seleccionar una carta como al guardar/agregar una respuesta en la grilla, via el `refreshHandler` de `CartaRespuestaTreeGridPanelV2`) recalcula el flag `cartaRespuestaSeleccionadaTieneRespuesta` revisando si la lista de cartas de respuesta cargada (`nivel=1`) contiene algun documento cuyo `id_documento_padre` coincide con `cartaRespuestaSeleccionada.getIdDocumentoAnalizado()`; `actualizarEstadoBotonRegistrarAsignacionCarta()` combina ese flag con la condicion ya existente (`cmbAbogadoCarta.getItemCount() > 0`) para habilitar/deshabilitar el boton. Ademas, `registrarAsignacionCarta()` valida el mismo flag antes de llamar al service (defensa en profundidad, por si el boton quedo habilitado un instante por una carrera entre las 2 cargas async independientes — abogados y cartas de respuesta — que corren en paralelo al seleccionar una fila), mostrando "Agregue la carta de respuesta en la grilla inferior antes de registrar la asignación." si no hay ninguna. Alcance: solo el boton de la lengueta "Cartas de respuesta" (`btnRegistrarAsignacionCarta`); el boton equivalente del panel "Documentos" (`btnRegistrarAsignacionDocumento`, `documentosTreePanel`) no se tocó, no fue parte de lo pedido.
- Plazo de vencimiento (columnas `Dias`/`Fecha Vencimiento`, calculado en `DocumentoAnalisisDAO.resolverVencimientoCarta`) en cascada segun que fecha ya este registrada: mientras no haya `Fecha Publ. Edicto`, corre desde `Fecha Acuse` o, si no hay acuse directo (el ciudadano se ubico via la Bandeja Publicacion de Notificacion en vez de un intento directo), desde `Fecha Publ. Notif.` — plazo de "respuesta" configurado por tipo de documento en Administracion > Plazos (`92_plazos_vencimiento_cartas_respuesta.sql`, ya ejecutado: 30 dias habiles para Carta Edicto/Precisar Pretension/Falta Sustento, 10 dias habiles para Carta Indagatorio). En cuanto se registra `Fecha Publ. Edicto`, el plazo cambia automaticamente a "publicacion" (15 dias habiles para Carta Edicto), contado desde esa fecha, sin esperar a que se derive el expediente a Analisis.
- Panel de busqueda (08/08/2026, pedido explicito del usuario: "el mismo diseño que la bandeja de asignacion"): mismo patron de 3 filas (`AppV2ExpedientePanelFactory.crearPanelBusquedaEstiloRegistro`) que la Bandeja Asignacion principal y Carga Abogados — texto libre + botones `Buscar`/`Limpiar`, `Fecha desde`/`Fecha hasta` (sobre `Fecha Vencimiento` de cada fila, ya expuesta en el DTO, sin consulta adicional), combo `Estado` (3 opciones propias de esta bandeja: `Todos los estados`, `Pendiente de Respuesta`, `Edicto Publicado`, las mismas 2 que puede devolver `estadoCartaRespuesta`) y limite numerico. Reemplaza el diseño anterior (`AppV2SearchToolbar` de una sola fila, con filtrado en vivo mientras se escribia y sin boton Buscar/fecha/estado). Todo el filtrado sigue siendo en memoria sobre `cartasRespuestaPendientes` (ya cargado completo por `listarCartasRespuestaPendientes`), ahora con clic explicito en `Buscar` en vez de disparar en cada tecla. `Fecha desde`/`Fecha hasta` traen valor por defecto al abrir el panel y al presionar `Limpiar` (08/08/2026, pedido explicito del usuario, exclusivo de esta bandeja): dia 01 del mes actual y fecha de hoy respectivamente (`DateRangePickerSupport.defaultSearchFromDateCurrentMonth()`/`defaultSearchToDate()`, mismo helper que ya usaban los campos de fecha de la Bandeja Asignacion principal, sin tocar esos campos).
...
- Estado `ANALISIS/DERIVADO` (08/08/2026, pedido explicito del usuario): expediente devuelto a Analisis desde la Bandeja Cartas de Respuesta de Asignacion (carta intermedia ya respondida por el ciudadano), distinto de `ANALISIS/OBSERVADO` (devuelto por una observacion real de Verificacion o del supervisor de Notificacion). Se comporta igual que `OBSERVADO` para todas las acciones operativas de Analisis (`AnalisisExpedienteDAO.registrarAnalisis`/`guardarDocumentosAnalisis`/`guardarDocumentosAnalisisJerarquicos`/`darBajaDocumentosAnalisis`/`enviarVerificacion` aceptan ambos estados por igual; `AnalisisExpedienteDTO.isRegistrable()`/`isEnviableVerificacion()` tambien), solo difiere en el rotulo/KPI. El KPI que antes era "Con carta intermedia" (heuristica `documentos>0 && !isEnviableVerificacion()`) ahora se llama "Derivado" y cuenta/filtra directamente por `estadoCodigo=DERIVADO` (`AnalisisExpedienteDTO.isDerivado()`), igual patron que el KPI ya existente "Observados". Ver seccion "Cartas de respuesta" (Asignacion) para el origen de la transicion y el script SQL asociado.
...
- `Publicacion` (4ta pestana, implementada 05/08/2026, ver seccion propia mas abajo).
- `Cierre` (lengueta interna del panel lateral de `Bandeja Notificacion`, no una pestana superior propia).
...
- Ciclo de un documento `FINAL`: En despacho -> Bandeja Asignacion (asignar validador) -> Bandeja Validacion (Aprobado/Observado) -> Aprobado reaparece en Bandeja Asignacion con el mini-panel "Emision" habilitado (pasa de Validado a Emitido, con numero de documento y fecha) -> tras emitirse, sigue visible en la misma Bandeja Asignacion (ahora en el mini-panel "Asignacion", segunda vez) para asignarlo al equipo de Notificacion -> Bandeja Notificacion. Si el validador marca Observado, el expediente se queda en `NOTIFICACION/POR_VALIDAR` (el resultado de validacion no mueve etapa/estado, solo limpia el responsable) y el documento reaparece en la misma Bandeja Asignacion (condicion ampliada, ver mas abajo) para que el supervisor lo derive con "Destino operativo".
- Ciclo de un documento `INTERMEDIO`: ya llega Emitido desde Verificacion; el mini-panel "Emision" no aplica (nunca se muestra) y solo se usa "Asignacion" para enviarlo al equipo de Notificacion.
- Panel fusionado "Asignacion" (antes 2 lenguetas separadas `Asignacion`/`Firma`) organizado como mini-paneles tipo stepper (`AppV2StepCardPanel`, ver abajo) segun el momento del documento enfocado, en vez de mostrar siempre las 4 secciones apiladas: **primer momento** (documento INTERMEDIO, o FINAL en `EN_DESPACHO`/`OBSERVADO`) solo muestra el mini-panel "Asignacion" (`Documentos seleccionados` + `Destino operativo`, sin numeracion); **segundo momento** (documento FINAL en `VALIDADO` o `EMITIDO`, es decir ya paso por Validacion) muestra 2 mini-paneles numerados: "① Emision" (`Documentos a firmar`, grilla `DocumentoFirmaNotificacionTreeGridPanelV2`) y "② Asignacion", bloqueada (candado + mensaje) hasta que exista el registro de "Registrar Supervision" en base de datos (07/08/2026, pedido explicito del usuario: ya no basta con que el documento este `EMITIDO`, ver bullet de "Mini-panel Emision" mas abajo). `Historial de asignacion/reasignacion` sigue siempre visible al final, sin condicionar por momento. Guardar la firma o generar una asignacion de un unico documento recarga la bandeja pero **reselecciona el mismo documento** (`cargarBandejaAsignacionNotificacion(Long)`/`reseleccionarDocumentoAsigNotif`), para que el desbloqueo del segundo mini-panel se vea en el mismo lugar sin que el usuario tenga que volver a buscarlo. Sin footer separado para Emision: el guardado de cada documento a firmar sigue siendo por fila (icono Guardar), y el footer `Generar asignacion`/`Cancelar` sigue existiendo solo para la asignacion a validador/notificador.
- `Destino operativo` (combo `Equipo destino`) ya no es solo `EQ_NOTIFICACION`/`EQ_VALIDACION`: `EQ_ANALISIS` y `EQ_EJECUCION` ahora derivan de verdad el expediente (antes estaban en el combo pero no hacian nada real). Exige exactamente 1 documento seleccionado (a diferencia de la asignacion a validador/notificador, que acepta lote), y solo pide abogado en el combo `Usuario destino` cuando el destino es Analisis (Ejecucion lo ignora: la regla es que la atienda el mismo abogado ya ligado via `EXPEDIENTE_ASIGNACION`, no el usuario elegido). Derivar a Analisis reutiliza `AsignacionExpedienteDAO.reasignarDesdeCartaRespuesta` (mismo metodo que el panel Cartas de Respuesta de Asignacion); derivar a Ejecucion usa el metodo nuevo `DocumentoAnalisisDAO.derivarDocumentoNotificacionAEjecucion`. Ambos dependen de transiciones de `FLUJO_TRANSICION` sembradas por el script `93_transicion_notificacion_destino_analisis_ejecucion.sql` (ya ejecutado) desde el unico origen real posible `NOTIFICACION/POR_VALIDAR`. El combo sigue mostrando siempre las 4 opciones sin filtrar por momento; `generarAsignacionNotificacion()` valida el equipo elegido contra el equipo esperado segun clasificacion+estado del documento (`equipoEsperadoParaClasificacionAsigNotif(clasificacion, estadoDocumentoCodigo)`), incluyendo el caso FINAL+EMITIDO -> EQ_NOTIFICACION de la segunda asignacion.
- Mini-panel "Emision": replica el diseno del panel `Verificar` de Verificacion, no solo la grilla, y al igual que Verificacion **nunca oculta secciones segun el resultado elegido** (corregido 05/08/2026: la version anterior ocultaba la grilla completa con Observado, contradiciendo el patron de Verificacion y el pedido explicito del usuario). Arriba, bloque "Resultado de Supervision" con combo Aprobado/Observado (fijo en codigo, sin catalogo de BD, igual que el combo Resultado de Verificar); por defecto Aprobado, y se reinicia a Aprobado cada vez que cambia el documento enfocado. La grilla `Documentos a firmar` (`DocumentoFirmaNotificacionTreeGridPanelV2`) esta **siempre visible**, sea Aprobado u Observado: lista TODOS los documentos del expediente del documento enfocado (no solo el documento cuya fila esta seleccionada en la bandeja, que en Asignacion es por-documento no por-expediente), columnas `Tipo documento`/`Numero Documento`/`Estado documento`/`Fecha Emision`. Numero, Fecha y Estado documento son editables, y solo en las filas elegibles para firma: `INTERMEDIO` siempre, `FINAL` solo si su estado es `VALIDADO` **o** `EMITIDO` (07/08/2026: antes se bloqueaba apenas pasaba a `EMITIDO`, impidiendo corregir el numero/fecha ya guardados; pedido explicito del usuario); las filas no elegibles se ven en gris con el icono Guardar deshabilitado y tooltip explicando el motivo. El combo de Estado documento (aqui y en la grilla de la Bandeja Validacion, ver mas abajo) ofrece `Emitido`/`En despacho`/`Observado`/`Validado` (`DocumentoAnalisisDAO.listarEstadosDocumentoNotificacion()`), a diferencia del catalogo de Analisis/Ejecucion que trae `En proyecto` en vez de `Validado`; el usuario puede ajustarlo ahi mismo (con su propio icono Guardar por fila) tanto en Aprobado como en Observado, sin que el resultado elegido se lo impida. **Observado** solo agrega debajo un bloque "Comentario" (obligatorio, motivo de la observacion), sin combo de equipo/usuario propio: ya no existe un "Destino operativo" duplicado dentro de Emision (eliminado 05/08/2026, pedido explicito del usuario: "eso ya se esta mostrando en el bloque inferior de asignacion"). En su lugar, mientras el resultado es Observado, `actualizarBloqueoAsignacionAsigNotif()` desbloquea el mini-panel "② Asignacion" (aunque el documento siga `VALIDADO`, no solo cuando ya esta `EMITIDO`) para que el supervisor use ahi mismo el combo `Equipo destino`/`Usuario destino` que ya soporta `EQ_ANALISIS`/`EQ_EJECUCION`. El boton "Registrar Supervision" (equivalente a "Registrar Verificacion" de Verificacion) NO vive dentro del contenido scrolleable del mini-panel: esta en el footer fijo compartido del panel completo, encima de "Generar asignacion" (mismo footer que ese boton y "Cancelar"), agregado/quitado dinamicamente segun el momento (oculto en el primer momento, donde Emision ni siquiera aplica) para no depender de `setVisible` en un `GridLayout` que igual reserva el espacio. Lee el combo Resultado: si Observado, valida que el equipo elegido en el combo compartido sea `EQ_ANALISIS`/`EQ_EJECUCION` + comentario, y reutiliza `derivarAsigNotifADestinoOperativo` (mismo metodo que usa el "Destino operativo" del mini-panel Asignacion; representa al supervisor de Asignacion observando el documento antes de emitirlo, distinto de cuando el validador ya lo observo, pero mismo mecanismo/destino final); si Aprobado, exige que el documento enfocado ya este `EMITIDO` (guardado con su icono de disco) y registra en `EXPEDIENTE_HISTORIAL` (tipo_movimiento `SUPERVISION_EMISION_NOTIFICACION`, `95_tipo_movimiento_supervision_emision_notificacion.sql`, no ejecutado) que la supervision fue aprobada, sin mover etapa/estado del expediente (eso ocurre recien en "② Asignacion"); antes (05/08/2026-07/08/2026) el resultado Aprobado no dejaba ningun rastro en base de datos, a diferencia de Analisis/Verificacion/Ejecucion, que si registran su resultado (07/08/2026, pedido explicito del usuario: "el resultado de supervision deberia registrarse en alguna parte"). El boton "Cancelar" del mismo footer, ademas de limpiar la seleccion/hoja de envio, cierra el panel (`cerrarPanelAsignacionNotif()`), igual que "Cancelar" en el modulo Asignacion.
- Bloqueo real de "② Asignacion" atado al registro de supervision, no solo al estado del documento (07/08/2026, pedido explicito del usuario): `DocumentoAnalisisDAO.existeSupervisionEmisionAprobada(idDocumentoAnalizado)` consulta (async, una vez por documento enfocado, cacheada en memoria hasta cambiar de documento) si ya existe la fila de `EXPEDIENTE_HISTORIAL` de `registrarSupervisionEmisionAprobada`. `actualizarBloqueoAsignacionAsigNotif()` calcula el bloqueo (`bloqueoAsignacionAsigNotif()`): con resultado Observado se desbloquea igual que antes (mismo mecanismo de siempre); con resultado Aprobado, permanece bloqueada hasta que ese registro exista. Ademas de bloquear/desbloquear la card, habilita/deshabilita los 2 botones del footer: `Generar asignacion` solo si hay documentos seleccionables y "② Asignacion" no esta bloqueada; `Registrar Supervision` se deshabilita una vez que la supervision Aprobada de ese documento ya quedo registrada (evita insertar el mismo registro de auditoria dos veces). Tras un `Registrar Supervision` exitoso, el flag se marca en caliente sin esperar una recarga completa, para que los botones/card reaccionen de inmediato.
- Bug "el panel se cierra solo" corregido definitivamente (07/08/2026), tercer intento tras 2 fixes previos (05/08 y 07/08, ver bullet siguiente) que igual no bastaban: la causa raiz real es que `tablaAsignacionNotif.setRowSelectionInterval(...)` dispara el `ListSelectionListener` generico de la tabla (`actualizarVisibilidadPanelAsigNotif()`), que solo puede REABRIR el panel si ya estaba visible en el instante exacto en que se dispara — y el `clearSelection()` de `poblarGrillaAsignacionNotif` (un instante antes, dentro del mismo refresco) ya lo habia cerrado, ganandole la carrera a cualquier intento de reabrirlo despues via la tabla. Fix definitivo: `reseleccionarDocumentoAsigNotif` ya no depende en absoluto de ese listener para este flujo — asigna `documentoAsigNotifFoco` directamente desde `documentosAsignacionNotif` (la lista completa recien recargada) y refresca los sub-paneles a mano (mismas llamadas que `actualizarFocoAsignacionNotif()`), dejando el `setSideVisible(true)` como ultimo paso garantizado, sin importar la vista filtrada actual.
- Bug corregido (05/08/2026): guardar la firma de un documento desde la grilla "Documentos a firmar" (`refreshHandler` de `crearDocumentosFirmaSeccionNotif`) recarga la bandeja y reselecciona el mismo documento via `cargarBandejaAsignacionNotificacion(Long)`/`reseleccionarDocumentoAsigNotif`, pero el recargo pasa primero por `tablaAsignacionNotif.clearSelection()` (dentro de `poblarGrillaAsignacionNotif`), que con el panel lateral visible en ese instante dispara `actualizarVisibilidadPanelAsigNotif()` y lo OCULTA; esa misma guarda solo puede ocultar (nunca reabrir) mientras el split ya esta oculto, asi que la reseleccion posterior encontraba y marcaba la fila correctamente pero el panel se quedaba cerrado (reportado por el usuario como "me saca del panel" al guardar un documento de carta intermedia dentro de la grilla). Fix: `cargarBandejaAsignacionNotificacion` captura si el panel estaba abierto (y no cerrado por el usuario) antes de recargar, y `reseleccionarDocumentoAsigNotif` fuerza `splitBandejasNotif.setSideVisible(true)` tras reseleccionar exitosamente si corresponde. Segunda causa distinta del mismo sintoma (07/08/2026): si el documento reselecciondo ya no aparece en la vista filtrada actual (p.ej. el filtro "Estado" de la busqueda quedo en un valor que el documento recien emitido ya no cumple), `reseleccionarDocumentoAsigNotif` no encontraba fila que seleccionar y el panel se quedaba cerrado sin reabrir; ahora hace fallback a `documentosAsignacionNotif` (la lista completa sin filtrar) para mantener el panel abierto sobre el mismo documento aunque haya salido de la vista filtrada.
- KPIs propios en la bandeja, con el mismo patron de filtros compactos de tres filas que Registro/Asignacion.
- Historial de asignaciones/reasignaciones de Notificacion se guarda en la tabla generica `EXPEDIENTE_HISTORIAL` (no una tabla nueva), con `tipo_movimiento` `ASIGNACION_NOTIFICACION`/`REASIGNACION_NOTIFICACION` (`58_tipo_movimiento_notificacion.sql`). El codigo Java que escribe (`DocumentoAnalisisDAO.asignarNotificacionMultiple`) y el que lee (`listarHistorialAsignacionesNotificacion`) estan correctamente conectados por `id_registro_relacionado = id_documento_analizado`; si el panel "Historial de asignacion / reasignacion" aparece vacio pese a haber asignaciones reales, sospechar primero de que el script 58 no se haya ejecutado contra la BD (sin esos codigos de `tipo_movimiento`, `asignarNotificacionMultiple` omite el INSERT en silencio, sin error visible para el usuario) antes de asumir un bug de UI.
...
- `Validar` incluye la grilla de documentos del expediente (`DocumentoEjecucionTreeGridPanelV2` en modo `configurarSoloComentarioEditable()`, compartido con Ejecucion; editable en Comentario y `Estado` con combo `Emitido`/`En despacho`/`Observado`/`Validado`) y el bloque `Resultado de validacion`: `Aprobado`/`Observado` (catalogo `tipo_resultado_validacion`, `57_catalogo_resultado_validacion_notificacion.sql`) con comentario obligatorio si es Observado. Un documento que ya tiene `Fecha Emision` registrada (`fecha_documento` no nulo) bloquea ademas la edicion de `Estado`, dejando editable unicamente Comentario (07/08/2026, pedido explicito del usuario: no se debe permitir tocar el resto de campos de un documento ya emitido).
...
- Si el intento 1 ya quedo Atendido/Ubicado (`NotificacionIntentoDTO.isUbicado()`, `estado_notificacion=EXITOSA`), `crearBorradoresIntento` ya no permite agregar el intento 2 (08/08/2026, pedido explicito del usuario): el documento ya fue notificado con exito, un 2do intento no tiene sentido. Mismo patron que el corte de maximo 2 intentos (arriba): se evalua por documento dentro del bucle de `crearBorradoresIntento`, asi que en una seleccion multiple los documentos ya ubicados se saltan sin bloquear al resto; si ningun documento de la seleccion califica (todos ya ubicados o ya en 2 intentos), se muestra un mensaje informativo en vez de agregar nada.
...
- Un documento cuyos intentos 1 y 2 ya fallaron (ambos `FALLIDA`) deja de listarse aqui de forma **definitiva** (08/08/2026, pedido explicito del usuario, ver bug real reportado con SDRERC-EXP-2026-000046 en la seccion "Bandeja Publicacion" mas abajo): pasa a mostrarse exclusivamente en la Bandeja Publicacion, incluso despues de que la publicacion ya se registre/marque `Publicado`. El filtro NO usa `estado_final_notificacion_codigo` (que pasa a `ATENDIDO` una vez publicado, lo que habria hecho que el documento "rebotara" de vuelta a esta bandeja); usa una bandera nueva e independiente `AGOTO_INTENTOS_DIRECTOS_SQL` (0/1, calculada solo con intento 1 y 2, sin mirar un eventual intento de PUBLICACION). `DocumentoAnalisisDAO.listarDocumentosNotificacion` llama `listarDocumentosNotificacionPareado(..., Boolean.FALSE)` (excluye los que agotaron); `listarDocumentosBandejaPublicacion` llama con `Boolean.TRUE` (solo los que agotaron).

Bandeja Publicacion (4ta pestana, implementada 05/08/2026):

- Mismo patron visual que `Bandeja Notificacion`: KPIs propios (`Documentos`, `Sin publicacion registrar`, `Publicacion registrada`, `Vencidos`), buscador compacto de 3 filas (texto/fechas/estado/limite, mismo componente `AppV2ExpedientePanelFactory.crearPanelBusquedaEstiloRegistro` que el resto de bandejas) y grilla arbol padre/hijo (documento + intentos) con icono de expandir/colapsar y checkbox de seleccion multiple en la fila padre.
- Fuente de datos: `DocumentoAnalisisDAO.listarDocumentosBandejaPublicacion(...)` (nuevo), mismo universo que `CONDICION_BANDEJA_NOTIFICACION` (misma visibilidad por asignacion), acotado a documentos que ya agotaron sus 2 intentos directos (`AGOTO_INTENTOS_DIRECTOS_SQL = 1`, ver bullet de bug corregido mas abajo — reemplaza el filtro original por `estado_final_notificacion_codigo = 'POR_PUBLICAR'`, que hacia que el documento desapareciera de esta bandeja apenas se marcaba Publicado). Implementado extendiendo `listarDocumentosNotificacionPareado(...)` con un parametro opcional `Boolean filtroAgotoIntentosDirectos` que envuelve la consulta pareada existente en `SELECT * FROM (...) WHERE agoto_intentos_directos = ?` (no se puede filtrar en el `WHERE` interno porque esa columna es una subconsulta correlacionada del `SELECT`, no una columna real de las tablas del `FROM`).
- La fila padre expandida muestra el/los intento(s) ya registrados, con las **mismas 7 columnas y los mismos valores que la Bandeja Notificacion** (08/08/2026, pedido explicito del usuario: "uniformizar... que me muestre las columnas como en notificacion, con los mismos valores"): `Nro. Intento`, `Modalidad`, `Fecha Envío`, `Estado`, `Código/Usuario Notif.`, `Fecha Acuse`, `Estado Notif./Public.` (renombrada, ver bullet siguiente), mas 1 columna propia al final (`Fecha Publicación`). Intento 1 y 2 se muestran **de solo lectura** (sin icono de accion, sin editores) con los mismos textos que en la Bandeja Notificacion (`Estado`: Enviado/Pendiente/Atendido; `Estado Notif./Public.`: Ubicado/No ubicado); `Código/Usuario Notif.` y `Fecha Acuse` vacios muestran `-`, igual que alli. El 3er registro es la "Publicacion" propiamente dicha: reutiliza el mismo mecanismo de intento (`EXPEDIENTE_NOTIFICACION`) con `tipo_notificacion='PUBLICACION'` (catalogo ya sembrado por `46_tipo_notificacion_publicacion.sql`), no una tabla nueva; se agrega **debajo de los intentos 1 y 2** (misma fila hija del arbol, ultima en orden). Para esta fila, `Modalidad` muestra `Publicación`, `Estado` queda fijo en `Enviado` (antes mostraba el nombre crudo del catalogo, p.ej. `Fallida`, que no es un valor valido de esa columna — pedido explicito del usuario: "estado fallido no existe"), y `Código/Usuario Notif.`/`Fecha Acuse` quedan en `-` (no aplican a un registro de publicacion). Los datos reales van en `Fecha Publicación` (editor calendario) y en la misma columna fusionada `Estado Notif./Public.` (combo `Pendiente`/`Publicado`, mapea a `estado_notificacion` codigos `PENDIENTE`/`EXITOSA`), con icono Guardar; antes de guardarse por primera vez aparece como fila "borrador" (icono Guardar + Cancelar, igual patron que `+ Agregar intento`), y ya guardada permite editar y tiene icono Eliminar (baja logica via `darBajaIntentoNotificacion`, mismo metodo que Notificacion). El campo libre "Referencia/Medio" que existia antes en la columna `Código/Usuario Notif.` se retiro de esta grilla (fijo en `-`) al uniformizar las columnas; se guarda vacio en `expediente_notificacion.codigo_notificacion` para este tipo de registro.
- Columna `Estado Publicación` eliminada y fusionada dentro de `Estado Notificación` (09/08/2026, pedido explicito del usuario: "eliminar la columna Estado Publicación... y ese estado de la publicación debe ser actualizado en la columna 'Estado Notificación'... así se evitan columnas innecesarias"). La columna renombrada pasa a llamarse `Estado Notif./Public.` y acepta 3 valores segun el tipo de fila: `Ubicado`/`No ubicado` para intento 1/2 (sin cambios, misma logica que ya usaba `Estado Notificación`), `Publicado`/`Pendiente` para la fila de Publicación (antes en la columna separada `Estado Publicación`, ahora eliminada). Solo cambio de UI: `JPanelNotificacionV2.COL_PUB_ESTADO_PUBLICACION` se elimino y `COL_PUB_ESTADO_NOTIF` pasa a ser la columna editable (combo `Pendiente`/`Publicado`) unicamente para la fila de Publicación (intento 1/2 la siguen mostrando de solo lectura); `PublicacionBandejaRenderer` decide que funcion de texto aplicar (`textoEstadoNotifHija` vs `textoEstadoPublicacion`) segun `fila.esIntentoSoloLectura()`. La columna `Estado Final` de la fila padre (`estado_final_notificacion_codigo`, calculada en `DocumentoAnalisisDAO`) ya reflejaba automaticamente `Publicado` como `Atendido` sin necesidad de cambio: esa formula ya trataba cualquier intento `EXITOSA` (directo o de publicacion) como atendido, asi que no hizo falta tocar SQL/DAO para que quedara consistente.
- `+ Agregar publicacion` (boton renombrado, mismo mecanismo que `+ Agregar intento`): inserta la fila "borrador" para cada documento marcado (o el enfocado con clic simple si no hay ninguno marcado); no aplica si el documento ya tiene una publicacion registrada o si ya alcanzo el maximo de 3 intentos.
- El documento permanece en esta bandeja despues de marcarse `Publicado` (08/08/2026, pedido explicito del usuario, corrige el comportamiento original documentado hasta el 07/08/2026 donde desaparecia al pasar a `ATENDIDO`): el filtro ya no depende de `estado_final_notificacion_codigo` (ver bullet de bug corregido mas abajo), asi que el registro sigue visible con su columna Estado Publicacion en `Publicado`, coherente con el KPI `Publicacion registrada` (que necesita tener registros que contar).
- Panel lateral (oculto por defecto, doble clic para abrir) con lenguetas `Datos` (mismo componente `DatosExpedienteNotifPanel` reutilizado, patron "Panel de datos" generico) y `Publicacion`: esta ultima es **puramente informativa** (tipo/numero/fecha de documento, estado final, resumen de intento 1/intento 2 y de la publicacion si ya existe), sin campos editables ni botones; editar/guardar/eliminar la publicacion se hace unicamente desde los iconos de la fila correspondiente en la grilla, pedido explicito del usuario.
- Pendiente (fuera de alcance sin autorizacion explicita de SQL): esta pestana no tiene codigo de permiso propio sembrado en `60_catalogo_permisos_bandejas.sql`, asi que por ahora queda habilitada para cualquier usuario que entre a Notificacion (sin `aplicarPermisoBandeja`); si se requiere gatearla por rol, hace falta un script nuevo que siembre el permiso y lo otorgue a `ADMIN_SISTEMA` antes de activar el gate (gatear con un codigo no sembrado dejaria la pestana inaccesible incluso para el admin).
- Bug corregido (08/08/2026, reportado con SDRERC-EXP-2026-000046, en 2 rondas): antes de este fix, la Bandeja Notificacion (3ra pestana) no excluia estos documentos en absoluto, por lo que el intento de publicacion (`tipo_notificacion='PUBLICACION'`) tambien podia llegar a mostrarse/guardarse como fila hija en esa 3ra pestana; si el usuario marcaba esa fila como "Ubicado"/"Atendido" desde ahi (`guardarFilaIntento`), disparaba `confirmarRecepcionIntentoNotificacion` (pensado solo para intentos directos 1/2) que sobreescribia `EXPEDIENTE_DOCUMENTO_ANALIZADO.fecha_acuse` — de ahi que la fecha de publicacion apareciera duplicada tanto en `Fecha Acuse` como en `Fecha Publ. Notif.` de la Bandeja Cartas de Respuesta (Asignacion). Fix de raiz: nuevo helper `JPanelNotificacionV2.filtrarIntentosDirectos(...)` excluye toda fila `tipo_notificacion=PUBLICACION` en los 3 puntos donde la 3ra pestana puebla `intentosNotifCache` (`alternarExpansionNotif`, `agregarIntentosInline`, `recargarBandejaYExpandir`), asi que esa fila nunca vuelve a ser visible ni editable desde la 3ra pestana, sea cual sea el estado del documento padre. Primera ronda del fix filtraba la bandeja por `estado_final_notificacion_codigo <> 'POR_PUBLICAR'`, pero eso permitia que el documento "rebotara" de vuelta a la 3ra pestana en cuanto la publicacion se marcaba `Publicado` (`estado_final_notificacion_codigo` pasa a `ATENDIDO`, que no es `POR_PUBLICAR`). Segunda ronda (definitiva): nueva bandera `AGOTO_INTENTOS_DIRECTOS_SQL`, calculada solo a partir de los intentos 1 y 2 (ignora si existe o no un 3er intento de publicacion y su resultado), usada en vez de `estado_final_notificacion_codigo` para decidir la bandeja — asi el documento queda permanentemente en Publicacion una vez que agota sus 2 intentos directos, publicado o no.
- Bug corregido (08/08/2026, tras uniformizar columnas): la columna `Fecha Publicación` parecia no guardarse (el `Estado Publicación` si). Causa: al armar la fila de Publicacion, el codigo leia `intento.getFechaPublicacion()` (campo de `EXPEDIENTE_PUBLICACION`, la tabla del modulo standalone "Publicacion", nunca escrita por esta fila) en vez de `intento.getFechaEnvio()` (campo real de `EXPEDIENTE_NOTIFICACION.fecha_envio` que `registrarIntentoNotificacion`/`actualizarIntentoNotificacion` si escriben). El guardado siempre funciono; solo la relectura mostraba el campo equivocado. Fix: usar `intento.getFechaEnvio()` para esta columna, igual que ya hacia `Fecha Publ. Notif.` de la Bandeja Cartas de Respuesta (`DocumentoAnalisisDAO.listarCartasRespuestaPendientes`, subconsulta ya correcta sobre `n2.fecha_envio` con `tipo_notificacion='PUBLICACION' AND estado='EXITOSA'`) — ambas columnas quedan consistentes sobre el mismo dato real, pedido explicito del usuario.
- `Estado Final` de la fila padre muestra `Publicado` en vez de `Atendido`, y `Fecha Publicación` de la fila padre ya no queda fija en `-` (09/08/2026, pedido explicito del usuario: "el estado final ya no sería Atendido sino Publicado... la columna Fecha Publicación no muestra nada debería mostrar la fecha de publicación"). Como esta bandeja solo lista documentos que ya agotaron sus 2 intentos directos (`AGOTO_INTENTOS_DIRECTOS_SQL = 1`), si `estado_final_notificacion_codigo` llega en `ATENDIDO` aqui solo puede deberse al intento de Publicación (nunca a un intento 1/2 exitoso, imposible en este universo); `JPanelNotificacionV2.textoEstadoFinalPublicacion(item)` traduce ese caso puntual a `Publicado` en la fila padre y en la lengueta informativa `Publicacion` del panel lateral, sin tocar `DocumentoAnalisisDAO.nombreEstadoFinalNotificacion` (mapeo compartido con las otras 3 pestañas de Notificación, donde `ATENDIDO` sigue significando ubicado directamente, sin cambios). Para `Fecha Publicación`, nueva columna calculada `DocumentoAnalisisDAO.FECHA_PUBLICACION_NOTIF_SQL` (mismo patron/subconsulta que `Fecha Publ. Notif.` de Cartas de Respuesta: `fecha_envio` del intento `PUBLICACION` ya `EXITOSA`) expuesta a nivel de documento (no solo al expandir), agregada a `listarDocumentosNotificacionPareado(...)` y al DTO compartido `NotificacionAsignacionDocumentoDTO.getFechaPublicacionNotif()` (nuevo campo, `null` en los otros 2 metodos del DAO que construyen este mismo DTO y no calculan esa subconsulta — `listarDocumentosPorExpediente`, `listarDocumentosNotificacionPorEstados` — sin impacto ahi).
- Al ordenar (clic en cabecera) esta bandeja o la Bandeja Notificación (3ra pestaña) con algun documento expandido, las filas hija ya no se desperdigan fuera de su fila padre (09/08/2026, pedido explicito del usuario: "la fila hija no se debería mover fuera de su padre sino que debe mantenerse dentro de su padre"). Causa: `AppV2ColumnFilterSupport` usa un `TableRowSorter` generico por celda sobre el modelo plano (padre+subencabezado+hijo en la misma tabla, con columnas superpuestas de distinto significado por tipo de fila); con `setSortsOnUpdates(true)`, cualquier reconstruccion del modelo mientras hay un orden de columna activo (expandir un documento, guardar/agregar un intento, recargar) reordena TODAS las filas visibles por valor de celda cruda, mezclando las filas de intentos con las de otros documentos. Fix: `JPanelNotificacionV2.limpiarOrdenSiHayExpandidos(...)`, invocado al inicio de `reconstruirFilasNotifBandeja()`/`reconstruirFilasPublicacionBandeja()`, limpia el orden activo del `TableRowSorter` (`setSortKeys(emptyList())`) cuando la reconstruccion va a incluir al menos un documento expandido — sin orden activo, el modelo conserva el orden de insercion, que ya agrupa correctamente cada padre con sus hijos por construccion. Efecto colateral aceptado: expandir cualquier documento mientras hay un ordenamiento de columna activo quita ese ordenamiento (hay que volver a hacer clic en la cabecera si se quiere re-ordenar); con todo colapsado, el ordenamiento por columna sigue funcionando igual que antes. Nuevo campo `columnFilterSupportPublicacionBandeja` (antes el `Controller` de `AppV2ColumnFilterSupport.install(...)` para esta bandeja se descartaba sin guardar) para poder acceder a su `TableRowSorter` desde este helper.
...
- `96_estado_derivado_analisis.sql`: siembra el estado `DERIVADO` en etapa `ANALISIS`, las transiciones `DEVOLUCION_A_ANALISIS` (desde `NOTIFICACION/EN_NOTIFICACION` y `NOTIFICACION/NOTIFICADO`, mismo patron del script 91 pero destino `DERIVADO`) y las transiciones `REGISTRO_RESULTADO_ANALISIS` con origen `DERIVADO` (5 destinos, replica del set ya sembrado para `OBSERVADO` en el script 20) (ver seccion "Cartas de respuesta" de Asignacion y seccion Analisis). No ejecutado.
...
- Congelamiento por carta intermedia pendiente (09/08/2026, pedido explicito del usuario, ver captura `docs/arquitectura_app/datos-del-plazo-cartas-de-respuesta.jpeg` — expediente de ejemplo SDRERC-EXP-2026-000005: "Días" debía congelarse en 28, no seguir bajando a 26): mientras el expediente tiene una carta intermedia (`TIPO_DOCUMENTO_ADJUNTO.CLASIFICACION='INTERMEDIO'`) activa, con `requiere_respuesta=1` y `fecha_documento` (emisión) ya registrada, pero **sin** confirmación de respuesta aún, el conteo de `Días` deja de correr contra "hoy" y se congela contra la fecha de emisión de esa carta (la más reciente si hay más de una pendiente). En cuanto se registra la confirmación de respuesta, el conteo vuelve a correr normalmente desde "hoy". `EXPEDIENTE.FECHA_VENCIMIENTO` (el plazo legal en sí) nunca se modifica: el congelamiento es solo de la fecha de referencia usada para calcular/mostrar `Días`, no una extensión real del plazo.
  - Implementado de forma centralizada en `CalendarioLaboralService`: nuevo overload `calcularDiasHabilesRestantes(Connection, Long idExpediente, Date fechaVencimiento)` (más su variante sin `Connection`), que resuelve la fecha de referencia (`resolverFechaReferenciaPlazo`/`resolverFechaEmisionCartaIntermediaPendiente`) antes de calcular; el overload sin `idExpediente` se mantiene (delega con `idExpediente=null`, sin congelamiento) solo como respaldo si algún llamador no puede resolver el id. Con un único punto de verdad, las ~12 pantallas/DAOs que muestran "Días" del expediente quedan consistentes automáticamente: `AsignacionExpedienteDAO` (panel de datos, incluida la vista de Cartas de Respuesta que reutiliza el mismo DTO/método), `AnalisisExpedienteDAO`, `VerificacionExpedienteDAO`, `EjecucionExpedienteDAO`, `NotificacionExpedienteDAO`, `DocumentoAnalisisDAO` (2 sitios, listados de documentos con fecha de vencimiento del expediente), `CierreArchivoDAO`, `ExpedienteDigitalDAO`, `PublicacionExpedienteDAO`, `ExpedienteRelacionadoDAO` (expedientes asociados), `FirmaEmisionExpedienteDAO`, `ExpedienteConsolaDAO`, `ExpedienteBandejaDAO` (Registro/Bandeja de Expedientes) y `UsuarioAsignacionDAO.listarDocumentosPorAbogado` (panel de detalle de Carga Abogados).
  - No afecta el plazo propio de las cartas de respuesta (`DocumentoAnalisisDAO.resolverVencimientoCarta`, columnas `Días`/`Fecha Vencimiento` de la grilla "Cartas de respuesta" de Asignación): esa es una cuenta regresiva distinta (10/15/30 días hábiles de la propia carta, no del expediente) y sigue calculándose siempre contra "hoy", sin congelamiento — congelar ese plazo no tendría sentido (la propia carta pendiente es la que dispara ese conteo, no algo que deba pausarlo).
  - Bug corregido (09/08/2026, mismo día, diagnosticado con `SELECT` de solo lectura autorizado por el usuario sobre SDRERC-EXP-2026-000005): la primera versión comparaba `confirmacion_respuesta IS NULL`, condición que en la práctica **nunca se cumple** — `DocumentoAnalisisDAO.normalizarConfirmacionRespuesta` normaliza cualquier valor vacío/no reconocido al literal `'PENDIENTE'` desde el primer guardado del documento en Análisis (se verificó con datos reales: `confirmacion_respuesta='PENDIENTE'`, nunca `NULL`), mucho antes de que el ciudadano responda; por eso el congelamiento nunca se activaba y el usuario seguía viendo 26 en vez de 28. Fix: la condición ahora es `UPPER(NVL(confirmacion_respuesta,'PENDIENTE')) = 'PENDIENTE'` (trata tanto `NULL` como el literal `'PENDIENTE'` como "todavía sin confirmar"); solo `'SI'`/`'NO'` (los otros 2 valores válidos de `normalizarConfirmacionRespuesta`) reactivan el conteo. **Nota para el futuro**: `UsuarioAsignacionDAO.CONDICION_CARTA_INTERMEDIA_RESPONDIDA` (KPI "Derivado"/"Carta intermedia" de Carga Abogados) sigue usando el `IS NOT NULL` original — no se tocó por estar fuera del alcance de este pedido, pero por el mismo hallazgo probablemente tenga el mismo problema (esa condición también sería casi siempre verdadera). Revisar si se reporta un síntoma relacionado ahí.
  - Icono de pausa junto al pill `Días` cuando el plazo está congelado (09/08/2026, pedido explicito del usuario): círculo amarillo con 2 barras verticales blancas (`JPanelAsignacionV2.PlazoCongeladoIcon`, dibujado a mano con `Graphics2D`, mismo patrón que otros iconos custom del archivo como `HeaderCheckBoxIcon`), con tooltip "Plazo congelado: esperando la respuesta del ciudadano a una carta intermedia ya emitida.". Implementado por ahora solo en el "Panel de datos" de `JPanelAsignacionV2` (cubre la lengueta `Datos` de Asignación y el panel de Cartas de Respuesta, que comparten el mismo método `actualizarDatosExpedientePanel`): nuevo método `CalendarioLaboralService.tienePlazoCongelado(...)` (reutiliza `resolverFechaEmisionCartaIntermediaPendiente`) expuesto vía `AsignacionExpedienteDAO`/`AsignacionExpedienteService.tienePlazoCongelado(idExpediente)`; `actualizarIconoPlazoCongelado(idExpediente)` lo consulta de forma asíncrona (`SwingWorker`, con guarda contra respuestas tardías si el usuario ya cambió de selección) y muestra/oculta el icono. No se replicó (todavía) en los demás paneles que muestran el pill `Días` (Registro, Análisis, Verificación, Ejecución, Notificación, Consola, Carga Abogados) — el pedido explícito fue "en el caso que te dije" (esta pantalla); extenderlo a las demás requeriría el mismo patrón repetido en cada panel.

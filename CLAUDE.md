...
- Congelamiento por carta intermedia pendiente (09/08/2026, pedido explicito del usuario, ver captura `docs/arquitectura_app/datos-del-plazo-cartas-de-respuesta.jpeg` — expediente de ejemplo SDRERC-EXP-2026-000005: "Días" debía congelarse en 28, no seguir bajando a 26): mientras el expediente tiene una carta intermedia (`TIPO_DOCUMENTO_ADJUNTO.CLASIFICACION='INTERMEDIO'`) activa, con `requiere_respuesta=1` y `fecha_documento` (emisión) ya registrada, pero **sin** `confirmacion_respuesta` aún (`IS NULL` — mismo campo/semántica que ya usa `UsuarioAsignacionDAO.CONDICION_CARTA_INTERMEDIA_RESPONDIDA` para saber si una carta "ya fue derivada de vuelta a Análisis"), el conteo de `Días` deja de correr contra "hoy" y se congela contra la fecha de emisión de esa carta (la más reciente si hay más de una pendiente). En cuanto se registra la confirmación de respuesta (`confirmacion_respuesta` deja de ser `NULL` — el mismo momento en que, en la práctica, "Registrar Asignación" de Cartas de Respuesta deriva el expediente a `ANALISIS/DERIVADO`), el conteo vuelve a correr normalmente desde "hoy". `EXPEDIENTE.FECHA_VENCIMIENTO` (el plazo legal en sí) nunca se modifica: el congelamiento es solo de la fecha de referencia usada para calcular/mostrar `Días`, no una extensión real del plazo.

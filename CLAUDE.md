...
- Columna `Estado` (07/08/2026, pedido explicito del usuario, reemplaza una version anterior basada en `confirmacion_respuesta`): `Derivado` cuando el documento ya tiene `Hoja de Envio` registrada (`EXPEDIENTE_DOCUMENTO_ANALIZADO.numero_hoja_envio_respuesta`, la misma columna "Hoja de Envio" de la grilla de Analisis) Y el expediente ya volvio a etapa `ANALISIS` (`JPanelAsignacionV2.estadoCartaRespuesta`, chequeado antes que las otras 2 ramas); `Edicto Publicado` una vez que se registra `Fecha Publ. Edicto` (hoy solo posible en Carta Edicto); `Pendiente de Respuesta` en cualquier otro caso (incluye Carta Edicto antes de publicar el edicto, y el resto de cartas intermedias — Carta Falta Sustento, Carta Precisar Pretension, Carta Indagatorio — que nunca llegan a tener `Fecha Publ. Edicto`).

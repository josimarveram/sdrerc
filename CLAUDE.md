...
- Un documento con `estado_final_notificacion_codigo = 'POR_PUBLICAR'` (ambos intentos directos ya fallaron) deja de listarse aqui (08/08/2026, pedido explicito del usuario, ver bug real reportado con SDRERC-EXP-2026-000046 en la seccion "Bandeja Publicacion" mas abajo): `DocumentoAnalisisDAO.listarDocumentosNotificacion` ahora llama `listarDocumentosNotificacionPareado(..., "POR_PUBLICAR", true)` (nuevo parametro `negarFiltroEstadoFinal`, invierte el filtro post-query de `=` a `<>`), de modo que ese documento pasa a mostrarse exclusivamente en la Bandeja Publicacion. `listarDocumentosBandejaPublicacion` sigue igual (filtro `=` sin negar).
...
- Fuente de datos: `DocumentoAnalisisDAO.listarDocumentosBandejaPublicacion(...)` (nuevo), mismo universo que `CONDICION_BANDEJA_NOTIFICACION` (misma visibilidad por asignacion), acotado a `estado_final_notificacion_codigo = 'POR_PUBLICAR'` (documento con intento 1 y 2 ya `FALLIDA`/no ubicado). Implementado extendiendo `listarDocumentosNotificacionPareado(...)` con un parametro opcional `filtroEstadoFinalPostQuery` que envuelve la consulta pareada existente en `SELECT * FROM (...) WHERE estado_final_notificacion_codigo = ?` (no se puede filtrar en el `WHERE` interno porque esa columna es una subconsulta correlacionada del `SELECT`, no una columna real de las tablas del `FROM`).
...
- Efecto colateral esperado (no requiere codigo adicional): al marcar la publicacion como `Publicado` (`EXITOSA`), `estado_final_notificacion_codigo` pasa a `ATENDIDO` (misma regla SQL que ya trataba cualquier intento `EXITOSA` como atendido) y el documento deja de aparecer en esta bandeja — comportamiento coherente con "ya resuelto via publicacion", no requiere estado nuevo.

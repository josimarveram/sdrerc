...
- La fila padre expandida muestra el/los intento(s) ya registrados: intento 1 y 2 se muestran **de solo lectura** (sin icono de accion, sin editores), ya que son los intentos normales de la Bandeja Notificacion. El 3er registro es la "Publicacion" propiamente dicha: reutiliza el mismo mecanismo de intento (`EXPEDIENTE_NOTIFICACION`) con `tipo_notificacion='PUBLICACION'` (catalogo ya sembrado por `46_tipo_notificacion_publicacion.sql`, pensado exactamente para esto: "3er intento... en la Bandeja de Publicacion"), no una tabla nueva. Es el unico registro editable de la grilla: columnas Fecha (editor calendario), Estado (combo `Pendiente`/`Publicado`, mapea a `estado_notificacion` codigos `PENDIENTE`/`EXITOSA`) y Referencia/Medio (texto libre, reutiliza `codigo_notificacion`), con icono Guardar; antes de guardarse por primera vez aparece como fila "borrador" (icono Guardar + Cancelar, igual patron que `+ Agregar intento`), y ya guardada permite editar y tiene icono Eliminar (baja logica via `darBajaIntentoNotificacion`, mismo metodo que Notificacion).

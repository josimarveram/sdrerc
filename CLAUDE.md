...
- `Validar` incluye la grilla de documentos del expediente (`DocumentoEjecucionTreeGridPanelV2` en modo `configurarSoloComentarioEditable()`, compartido con Ejecucion; editable en Comentario, Numero y `Estado` con combo `Emitido`/`En despacho`/`Observado`/`Validado`) y el bloque `Resultado de validacion`: `Aprobado`/`Observado` (catalogo `tipo_resultado_validacion`, `57_catalogo_resultado_validacion_notificacion.sql`) con comentario obligatorio si es Observado.
